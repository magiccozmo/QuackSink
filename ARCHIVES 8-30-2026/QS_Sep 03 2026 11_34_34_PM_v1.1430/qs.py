...
#
# QS 1.1430:
#   - Preserves QS 1.0 relay/DOM behavior.
#   - Fixes GitHub archive URLs containing spaces.
#   - Uses explicit URL encoding for archive names.
#   - Reports repository fetch failures separately from true mismatches.
#   - Reports local and remote SHA256 hashes during verification.
#   - A true hash mismatch still halts startup.
#   - A repository/network verification failure does NOT falsely report
#     a source mismatch.
#   - First-run behavior remains compatible with archive creation.
#   - Adds archive-local .title files through the SAVE TITLE control.
#   - Adds TOP/BOTTOM relay-display navigation controls.
#   - Returns the relay display to TOP after Fan-Out completes.
#   - Uses protocol tokens only during the initial handshake.
#   - Post-handshake valid responses do not require framing tokens.
#
# IMPORTANT:
#   The QS 1.0 archive remains untouched.
#   QS 1.1430 is a new runtime version.
...
class QSUI:
    ...
    def __init__(self, controller):
        ...
        self.root.title(
            f"QuackSink QS {VERSION} - Structural DOMAnalyzer"
        )
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self.root,
            text=f"QUACKSINK - MULTI-MIND RELAY CORE - QS {VERSION}",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(pady=(10, 0))
    ...
    def _save_title(self):
        ...
        # IMPORTANT: append .title FIRST, then validate the resulting filename.
        ...
        if invalid_reason:
            ...
            self.log(
                f"[TITLE] Rejected: {invalid_reason}",
                is_system=True
            )
        ...
        if os.path.exists(path):
            ...
            message = (
                f"Title file already exists in the current archive:\n\n"
                f"{filename}"
            )

            self.log(
                f"[TITLE] Rejected: {filename} already exists.",
                is_system=True
            )
        ...
        try:
            ...
            self.log(
                f"[TITLE] Could not create {filename}: {exc}",
                is_system=True
            )

            messagebox.showerror(
                "SAVE TITLE",
                f"Could not create title file:\n\n{exc}"
            )

            return

        self.log(
            f"[TITLE] Saved: {filename}",
            is_system=True
        )

        messagebox.showinfo(
            "SAVE TITLE",
            f"Title saved in the current archive:\n\n{filename}"
        )
    ...
    def _validate_title_filename(filename):
        ...
        if any(
            ...
            return (
                'Invalid filename: the characters < > : \" / \\ | ? * '
                "are not allowed."
            )

        if filename.endswith(" ") or filename.endswith("."):
            return "Invalid filename: it cannot end with a space or period."
        ...
        if stem in reserved:
            return f"Invalid filename: '{stem}' is a reserved Windows name."

        if len(filename) > 255:
            return "Invalid filename: it is longer than 255 characters."
    ...
    def _copy_selected_source(self):
        ...
        if not os.path.isfile(source_path):
            ...
            self.log(
                f"[CLIPBOARD] qs.py not found: {source_path}",
                is_system=True
            )
...
class QSController:
    ...
    async def _response_frame_state(
        ...
        # After the initial handshake, tokens are no longer required.
        # Any genuinely new non-empty assistant message is a valid response.
    ...
    def _parse_response(
        ...
        # Tokens are protocol framing only for the initial handshake.
        # Keep compatibility with a framed handshake response, but do not
        # require or strip tokens from ordinary post-handshake messages.
        matches = list(
            re.finditer(
                r"🟩.*?🔚",
                trimmed,
        ...
        # Ordinary post-handshake response: the complete message is valid.
        # No token is required, and no token framing is expected.
    ...
    def _on_relay_change(
        ...
        if relay.state == RelayState.COMPLETE:
            ...
            # Fan-Out is complete: return the relay display viewport to TOP.
            self.ui.root.after(
                0,
                self.ui.scroll_top
            )
