...
#   - QS 1.1430 adds operator title-file creation in the active archive.
#   - QS 1.1430 adds TOP/BOTTOM relay display navigation and automatic TOP after Fan-Out.
#   - QS 1.1430 keeps token framing for the initial handshake only.
#   - After handshake, non-empty responses are relayed as ordinary messages without token parsing.
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        self.title_entry = tk.Entry(
            title_frame, font=("Arial", 13, "bold"),
            bg="#1e1e1e", fg="#ffffff", width=40,
            insertbackground="white"
        )
        self.title_entry.pack(side=tk.LEFT, padx=(0, 8))

        tk.Button(
            title_frame, text="SAVE TITLE", command=self._save_title,
        ...
        nav_frame = tk.Frame(self.root, bg="#121212")
        nav_frame.pack(pady=(0, 3))
        tk.Button(
            nav_frame, text="TOP", command=self.scroll_top,
            bg="#555555", fg="white", font=("Arial", 11, "bold"),
            height=1, padx=12
        ).pack(side=tk.LEFT, padx=4)
        tk.Button(
            nav_frame, text="BOTTOM", command=self.scroll_bottom,
            bg="#555555", fg="white", font=("Arial", 11, "bold"),
            height=1, padx=12
        ).pack(side=tk.LEFT, padx=4)
    ...
    @staticmethod
    def _validate_title_filename(filename):
        if not filename or filename == ".title":
        ...
        if any(ch in '<>:"/\\|?*' for ch in filename):
            return "Title contains a Windows-invalid filename character."
        if filename.endswith(" ") or filename.endswith("."):
            return "Title cannot end with a space or period."
        if len(filename) > 255:
            return "Title filename is too long."
        stem = filename[:-6] if filename.lower().endswith(".title") else filename
        reserved = {
            "CON", "PRN", "AUX", "NUL",
        ...
        if stem.upper() in reserved:
            return "Title uses a reserved Windows filename."
        return None

    def _save_title(self):
        raw_title = self.title_entry.get().strip()

        # Required order: append .title FIRST, then validate the resulting filename.
        filename = raw_title + ".title"
        invalid_reason = self._validate_title_filename(filename)
        if invalid_reason:
            self.log(f"[TITLE] Rejected: {invalid_reason}", is_system=True)
            messagebox.showerror("Invalid Title", invalid_reason)
            return

        path = os.path.join(ACTIVE_ARCHIVE_DIR, filename)
        if os.path.exists(path):
            reason = f"Title already exists in the current archive: {filename}"
            self.log(f"[TITLE] Rejected: {reason}", is_system=True)
            messagebox.showerror("Title Already Exists", reason)
            return
        ...
        try:
            ...
            reason = f"Could not create title file: {exc}"
            self.log(f"[TITLE] ERROR: {reason}", is_system=True)
            messagebox.showerror("Save Title Failed", reason)
            return

        self.log(f"[TITLE] Saved: {filename}", is_system=True)
        self.title_entry.delete(0, tk.END)
        messagebox.showinfo("Title Saved", f"Created empty title file:\n{filename}")
...
class QSController:
    ...
    async def _wait_for_response(self, page, node, timeout=45):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            if frame_ready:
                ...
                if frame_streak >= 2:
                    self.ui.log(
                        f"[WAIT] {node.name}: complete NEW response detected.",
                        is_system=True
                    )
                    return True
            else:
    ...
    def _on_relay_change(self, relay):
        ...
        if relay.state == RelayState.COMPLETE:
            ...
            self.ui.root.after(0, self.ui.scroll_top)
