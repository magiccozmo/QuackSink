...
#
# Luma 2.0:
#   - Repairs response completion detection.
#   - Does NOT treat send-button readiness alone as proof of a response.
#   - Requires a NEW assistant DOM message before accepting a response.
#   - Adds response-DOM diagnostics when waiting fails.
#   - Preserves dropout handling.
#   - A response containing 🛑 is treated as a dropout, even without
#     a normal 🟩...🔚 frame.
...
#   We are fixing the GPT response-detection problem before adding
#   additional MMRC features.
...
logger.info("=============================================================")
logger.info("MMRC Luma 2.0 STARTING")
...
class Node:
    def __init__(self, name, url, icon="🔮"):
        ...
        # Snapshot of assistant output immediately before submission.
...
class MMRCUI:
    ...
    def __init__(self, controller):
        ...
        self.root.title(
            "MMRC Luma 2.0 - Response Detection Repair"
        )
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self.root,
            text="MULTI-MIND RELAY CORE - Luma 2.0",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(pady=(10, 0))
...
class MMRCController:
    ...
    async def _send_message_to_page(
        ...
        try:
            ...
            # CRITICAL:
            # Capture assistant output BEFORE submission.
    ...
    async def _response_frame_state(
        ...
        """
        Return a structured response state.

        IMPORTANT Luma 2.0 rule:
        send-button readiness is NOT enough.

        We need to observe an actual NEW assistant message.
        ...
        elif messages != baseline:

            # Same number of DOM messages, but
    ...
    async def _wait_for_response(
        ...
        """
        Luma 2.0 response waiter.

        The old logic could return success merely because
        the send button became ready after we had observed
        a busy state.

        That is unsafe.

        Luma 2.0 requires an actual NEW assistant message.

        If the new message contains:
            🟩 ... 🔚
        it is a complete MMRC frame.

        If the new message contains:
            🛑
        it is a dropout.

        We still monitor send readiness because it is useful
        diagnostic information, but it cannot independently
        declare success.
        """

        self.ui.log(
            f"[WAIT] {node.name}: "
            f"waiting for NEW assistant response...",
            is_system=True
        )
        ...
        while (
            ...
            try:
                ...
                # A NEW assistant message exists, even if it
                # has not yet acquired its final MMRC delimiters.
                #
                # We deliberately do not accept it immediately,
                # because generation may still be underway.
                #
                # Continue polling until it becomes a frame or
                # dropout.
        ...
        try:
            ...
            self.ui.log(
                f"[DIAG] {node.name}: "
                f"baseline messages={len(baseline)}, "
            ...
            ready = await self._send_button_state(
                page
            )
    ...
    def _parse_response(self, text):
        ...
        # Dropout is now independently recognized.
        #
        # This permits a node to say simply:
        #
        #     🛑
        #
        # or:
        ...
        #     🛑 I am leaving.
        #
        # without having to manufacture an MMRC frame.
        if "🛑" in trimmed:
