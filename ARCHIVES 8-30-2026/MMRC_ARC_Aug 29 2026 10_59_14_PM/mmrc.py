...
#
# Luma 2.5 additions/fixes:
#   - Fixed truncated domain fallback code from Luma 2.4.
#   - Logger initialization occurs before any startup logging.
#   - Startup archive remains the first operation.
...
#   - Selected archive is presentation-only; real filesystem path retained.
...
logger.info("=============================================================")
logger.info("MMRC Luma 2.5 STARTING")
...
class DOMAnalyzer:
    ...
    async def analyze(page: Page):
        ...
        The analyzer deliberately asks the browser to construct selectors
        rather than blindly joining raw class names.

        Output selectors describe the MESSAGE SET, not one historical
        message, so they remain useful after new responses arrive.
        """
...
class SelectorDetector:
    ...
    def generate_name(url):
        ...
        # ---------------------------------------------------------------
        # IMPORTANT:
        # This is the block that was accidentally truncated in 2.4.
        # ---------------------------------------------------------------
...
class MMRCUI:
    ...
    def __init__(self, controller):
        ...
        self.root.title(
            "MMRC Luma 2.5 - Structural DOMAnalyzer"
        )
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self.root,
            text="MULTI-MIND RELAY CORE - Luma 2.5",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(pady=(10, 0))
...
class MMRCController:
            ...
            async with async_playwright() as p:
                ...
                for node in self.relay.active_nodes:
                    ...
                    if not success:
                        ...
                        self.ui.log(
                            f"[RELAY] Failed to send to {node.name}.",
                            is_system=True
                        )
    ...
    async def _send_message_to_page(
        ...
        try:
            ...
            if not selector:
                ...
                self.ui.log(
                    f"[SEND] No input selector for {node.name}.",
                    is_system=True
                )
            ...
            if not submit_success:
                ...
                self.ui.log(
                    f"[SEND] SUBMIT failed for {node.name}.",
                    is_system=True
                )
    ...
    async def _wait_for_response(
        ...
        while (
            ...
            candidates = (
                messages_now[len(baseline):]
                if len(messages_now) > len(baseline)
                else []
            )

            if (
                not candidates and
                messages_now[-1:] != baseline[-1:]
            ):
            ...
            if (
                saw_new_message
                and new_message_stable_streak >= 2
                and ready_streak >= 2
            ):

                self.ui.log(
                    f"[WAIT] {node.name}: "
                    f"new response finalized; "
                    f"checking MMRC framing.",
                    is_system=True
                )
