...
RUNTIME_FILE = "qs6_fixed_v6_5.py"
...
class QSController:
    ...
    async def _click_claude_read_aloud(self, page, node):
        """Claude: click the actual Read Aloud control in the latest response action row.

        Claude's current UI exposes Read Aloud as an exact semantic control in the
        response action row (alongside controls such as Copy and feedback). We bind
        the click to the latest Claude response rather than using a page-wide button
        or a geometry-based guess.
        """
        try:
            ...
            result = await page.evaluate(rf"""
                (() => {{
                    ...
                    // Prefer leaf nodes so the chosen response is the actual content-bearing
                    // response rather than a wrapper containing the whole conversation.
                    ...
                    if (!target) {{
                        ...
                        if (nearby.length) target = nearby[nearby.length - 1];
                    }}

                    if (!target) {{
                        const diagnostics = Array.from(document.querySelectorAll(
                            'button,[role="button"]'
                            ...
                            el.getAttribute('data-testid') || '',
                            el.textContent || ''
                        ].join(' | '))).filter(Boolean).slice(-50);
                        return {{ok:false, stage:'read_aloud', reason:'Exact Claude Read Aloud control not found.', diagnostics}};
                    }}

                    target.click();
                    return {{ok:true, label:labels(target).join(' | ')}};
                }})()
            """)
        except Exception as exc:
        ...
        if not result or not result.get('ok'):
            ...
            self.ui.log(f"[VOICE] Claude Read Aloud unavailable: {reason}", is_system=True)
            if result and result.get('diagnostics'):
                self.ui.log(
                    f"[VOICE] Claude nearby button diagnostics: {result.get('diagnostics')}",
                    is_system=True
                )
            return False

        self.ui.log(
            f"[VOICE] Claude Read Aloud clicked (label={result.get('label')!r}).",
            is_system=True
        )
        return True
    ...
    async def _gpt_voice_state(self, page):
        """Return the current GPT native Read Aloud state for the latest response.

        Returns one of: ``active``, ``idle``, or ``unknown``.

        GPT's native Read Aloud action can expose different controls while playback is
        running and after it finishes. The important rule is that only the latest
        response's own action area counts. Generic page-wide Stop/Pause controls and
        scroll controls must never hold the relay hostage.
        """
        try:
            result = await page.evaluate(r"""
                (() => {
                    ...
                    if (!latest) return {state:'unknown', reason:'latest response not found'};
                    ...
                    const active = controls.find((el) => labels(el).some((value) =>
                        /^(?:stop\s+reading(?:\s+aloud)?|pause\s+reading(?:\s+aloud)?|stop\s+(?:speech|speaking|playback)|pause\s+(?:speech|speaking|playback))$/i.test(value) ||
                    ...
                    try {
                        audioActive = Array.from(document.querySelectorAll('audio')).some((audio) =>
                            !audio.paused && audio.readyState >= 2
                        );
                    } catch (_) {}
                    if (audioActive) return {state:'active', label:'audio-element'};

                    try {
                        if (window.speechSynthesis && window.speechSynthesis.speaking) {
                            return {state:'active', label:'speechSynthesis'};
                        }
                    } catch (_) {}

                    return {state:'unknown',
                            reason:'No GPT Read Aloud active/idle control or active audio signal detected.'};
                })()
            """)
    ...
    async def _gpt_voice_stop_visible(self, page):
        """Compatibility wrapper: True only while GPT native playback is active."""
        state = await self._gpt_voice_state(page)
    ...
    async def _claude_voice_stop_visible(self, page, node):
        """Return True while Claude's latest response exposes active playback.

        Claude's Read Aloud control becomes Pause during playback and returns
        to Read Aloud when playback finishes.
        """
        try:
            submitted = self._canonical_message_text(node.last_submitted_text)
            safe_selector = json.dumps(
            ...
            return bool(await page.evaluate(fr"""
                (() => {{
                    ...
                    const isActivePlayback = (el) => visible(el) && labels(el).some((value) =>
                        /^(?:stop|pause|speaking|playback)$/i.test(value) ||
                        /(?:stop|pause)\\s+(?:speech|speaking|playback)/i.test(value)
                    );
                    const submittedText = {safe_submitted};
                    const messages = [];
                    const seen = new Set();
                    ...
                    try {{ document.querySelectorAll({safe_selector}).forEach((el) => add(el, 100)); }} catch (_) {{}}
                    for (const sel of [
                        '[data-testid*="assistant-message"]','[data-testid*="message-assistant"]',
                        '[data-testid*="response"]','.font-claude-response','.font-claude-response-body',
                        '.font-claude-message','.assistant-message','[role="article"]','article'
                    ]) {{
                        try {{ document.querySelectorAll(sel).forEach((el) => add(el, 50)); }} catch (_) {{}}
                    }}
                    if (!messages.length) return false;
                    const leaves = messages.filter((item) => !messages.some((other) =>
                        other.el !== item.el && item.el.contains(other.el)
                    ));
                    const pool = leaves.length ? leaves : messages;
                    let candidates = pool.filter((item) => canonical(item.text) !== submittedText);
                    if (!candidates.length) candidates = pool;
                    candidates.sort((a, b) => {{
                        const pos = a.el.compareDocumentPosition(b.el);
                        if (pos & Node.DOCUMENT_POSITION_FOLLOWING) return -1;
                    ...
                    const scopes = [latest];
                    let ancestor = latest.parentElement;
                    for (let i = 0; i < 5 && ancestor; i += 1) {{ scopes.push(ancestor); ancestor = ancestor.parentElement; }}
                    for (const scope of scopes) {{
                        const controls = Array.from(scope.querySelectorAll(
                            'button,[role="button"],[data-testid],[aria-label],[title],[data-tooltip],[data-tip]'
                        )).filter((el) => !el.closest('[role="menu"],nav,header'));
                        if (controls.some(isActivePlayback)) return true;
                    }}
                    return false;
                }})()
            """))
    ...
    async def _wait_for_voice_playback(self, page, node):
        """Wait for the node's active playback control to appear, then disappear.

        Important: an idle "Read aloud" button is not playback. Active speech is
        represented by Stop/Pause/Speaking-style state, or a demonstrably active
        audio/speech-synthesis signal.
        """
        start = asyncio.get_event_loop().time()
        saw_playback = False
        saw_native_activity = False
        gpt_active_seen = False
        idle_clear_streak = 0

        def elapsed():
            return asyncio.get_event_loop().time() - start

        # Native UI playback can take longer than a few seconds to expose its Stop/Pause
        # state. Give the browser enough time to enter its speaking state instead of
        # allowing the relay to advance while audio is still starting.
        while elapsed() < 30.0:
            if node.name == 'GPT':
                gpt_state = await self._gpt_voice_state(page)
                if gpt_state.get('state') == 'active':
                    gpt_active_seen = True
                    saw_playback = True
                    self.ui.log(
                        f"[VOICE] GPT playback started; state={gpt_state.get('label', 'active')!r}; waiting for native playback to finish.",
                        is_system=True
                    )
                    break
            else:
                if node.name == 'Aisha':
                    control_visible = await self._aisha_voice_stop_visible(page, node)
                elif node.name == 'Claude':
                    control_visible = await self._claude_voice_stop_visible(page, node)
                else:
                    control_visible = False

                native_activity = await self._native_voice_activity_visible(page)
                if control_visible or native_activity:
                    saw_playback = True
                    saw_native_activity = saw_native_activity or native_activity
                    self.ui.log(
                        f"[VOICE] {node.name} playback started; waiting for native playback to finish.",
                        is_system=True
                    )
                    break
        ...
        if not saw_playback:
            self.ui.log(
                f"[VOICE] {node.name} native playback state was not observed within 30s; "
                "refusing to advance silently.",
                is_system=True
            )
            return False

        # Once playback is observed, do not release the serialized relay turn until the
        # node's speaking state has cleared. The generous safety ceiling is only a guard
        # against a broken browser control, not the normal completion path.
        while elapsed() < 1800.0:
            if node.name == 'GPT':
                ...
                if gpt_state.get('state') == 'active':
                    gpt_active_seen = True
                    idle_clear_streak = 0
                elif gpt_active_seen and gpt_state.get('state') == 'idle':
                    idle_clear_streak += 1
                    if idle_clear_streak >= 2:
                        self.ui.log("[VOICE] GPT playback complete; Read Aloud control returned to idle.", is_system=True)
                        return True
                else:
                    idle_clear_streak = 0
            else:
                ...
                native_activity = await self._native_voice_activity_visible(page) if saw_native_activity else False
                if not control_visible and not native_activity:
    ...
    async def _maybe_speak_node(self, page, node, response_text):
        ...
        if node.name == 'GPT':
            ...
            started = await self._click_claude_read_aloud(page, node)
        else:
            started = False
