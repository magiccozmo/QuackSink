...
# QS 4.39 DEVELOPMENT:
#   - Rebuilds Aisha support from pristine QS 4.37.
#   - Keeps the generic browser capture/send path unchanged.
#   - Aisha is handled only through a node-specific structural capture adapter.
#   - The adapter follows the recovered DRS/MMRC workaround: read the latest
#     child of the structural assistant/response container, rather than re-reading
#     the entire transcript/history wrapper.
#
# QS 4.40 DEVELOPMENT:
#   - Restores the proven QS 4.19 target-page CDP Enter submission fallback.
#   - Replaces synthetic DOM KeyboardEvent Enter dispatch, which can clear the composer
#     without triggering the site's real submit handler.
#   - Leaves the Aisha-specific latest-child response capture from QS 4.39 unchanged.
...
class QSController:
    ...
    async def _maybe_speak_node(self, page, node):
        ...
        if node.name == 'GPT':
            ...
            self.ui.log(f"[VOICE] {node.name} voice path is unavailable in QS 4.37.", is_system=True)
            return
    ...
    async def _send_message_to_page(self, page, node, text, is_handshake):
        ...
        try:
            ...
            node.response_baseline = await self._capture_node_snapshot(page, node)
            self.ui.log(
    ...
    async def _submit_verified(self, page, node, input_element, selector, method):
        ...
        while asyncio.get_event_loop().time() - start < timeout:
            ...
            try:
                ...
                busy = False
            current = await self._capture_node_snapshot(page, node)
    ...
    async def _submit(self, page, node, input_element, selector):
        ...
        # Final fallback: send a real Enter key through the target page's
        # CDP input path. This is the known-good QS 4.19 submission mechanism.
        # It targets the exact page and does not require the browser window to
        # own OS focus; unlike synthetic DOM KeyboardEvent dispatch, the event
        # reaches the web app as a browser-level keypress.
        try:
            await input_element.focus()
        except Exception:
            pass
        ...
        try:
            self.ui.log(f"[SEND] SUBMIT ATTEMPT (Enter/CDP): {node.name}", is_system=True)
            await asyncio.to_thread(
                self.browser.bridge.request,
                {"op": "target_key_press", "pageId": node.page_id, "url": node.url, "key": "Enter"}
            )
            if await self._submit_verified(page, node, input_element, selector, "Enter"):
                return True
        except Exception as exc:
            self.ui.log(f"[SEND] Enter CDP submit attempt failed for {node.name}: {exc}", is_system=True)
    ...
    async def _capture_aisha_latest(self, page, node):
        """Aisha-specific locked capture: read the newest structural child only.

        Recovered DRS/MMRC behavior: Aisha's UI can re-ingest the whole transcript
        wrapper during parsing. The old workaround therefore treated the structural
        assistant/response container as authoritative and read only its latest child.

        This function is deliberately NOT part of the generic browser capture path.
        """
        ...
        try:
            result = await page.evaluate(
                ...
                (selector) => {
                    ...
                    if (!containers.length) {
                        return {ok: false, reason: 'Aisha response container not found.'};
                    }

                    // The recovered workaround expects the selector to identify the
                    // structural response container. Prefer its newest direct child.
                    ...
                    if (children.length) {
                        ...
                        if (text) {
                            return {ok: true, text, mode: 'last-child'};
                        }
                    }

                    // Some Aisha DOM revisions expose individual response nodes
                    // rather than one container. In that case, use the newest match.
                    const lastMatch = containers[containers.length - 1];
                    const fallbackText = String(lastMatch.innerText || lastMatch.textContent || '').trim();
                    if (fallbackText) {
                        return {ok: true, text: fallbackText, mode: 'last-match'};
                    }

                    return {ok: false, reason: 'Latest Aisha response node is empty.'};
                }
                """,
                selector
            )
            ...
            if not result or not result.get('ok'):
                self.ui.log(
                    f"[AISHA-CAPTURE] No latest response: {result.get('reason', 'unknown') if result else 'no result'}",
                    is_system=True
                )
                return []

            captured = str(result.get('text') or '').strip()
            if not captured:
                return []

            self.ui.log(
                f"[AISHA-CAPTURE] Locked to {result.get('mode', 'latest')} response node; chars={len(captured)}.",
                is_system=True
            )
            return [captured]
    ...
    async def _capture_node_snapshot(self, page, node):
        """Dispatch capture by node without altering the generic QS adapter."""
        if node.name == 'Aisha':
            return await self._capture_aisha_latest(page, node)
        return await self._capture_output_snapshot(page, node)
    ...
    async def _response_frame_state(self, page, node):
        messages = self._normalize_messages(
            await self._capture_node_snapshot(page, node),
            "response_frame",
        )
    ...
    async def _get_response(self, page, node, is_handshake=False):
        messages = await self._capture_node_snapshot(page, node)
        if not messages:
