...
RUNTIME_FILE = "qs6_fixed_v6_4.py"
...
class QSController:
            ...
            return bool(await page.evaluate(r"""
                (() => {
                    ...
                    const isPlayback = (el) => {
                        ...
                        return /^(?:stop(?:\s+reading(?:\s+aloud)?)?|speaking|pause(?:\s+reading(?:\s+aloud)?)?)$/i.test(label) ||
    ...
    async def _wait_for_voice_playback(self, page, node):
        ...
        while elapsed() < 30.0:
            if node.name == 'GPT':
                control_visible = await self._gpt_voice_stop_visible(page)
