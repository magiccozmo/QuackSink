...
#   - Voice starts OFF and remains unavailable until a completed normal GPT relay
#     response provides a real Read aloud control to probe.
#   - Once GPT Read aloud is found, the VOICE toggle becomes available; capability
#     probing never silently turns an already-enabled voice setting OFF.
#   - If a later enabled Read aloud attempt fails, voice automation is disabled
#     while the relay itself continues normally.
...
class QSUI:
    ...
    def set_voice_state(self, available, enabled):
        try:
            self.controller.voice_available = bool(available)
            self.controller.voice_enabled = bool(enabled) if available else False
            if available:
                if self.controller.voice_enabled:
                ...
                self.voice_toggle_btn.config(
                    text="VOICE: OFF (GPT READ ALOUD NOT FOUND)",
...
class QSController:
    def __init__(self):
        ...
        # Voice always starts OFF and unavailable.  Capability is discovered
        # only from a completed GPT response.
    ...
    async def _do_scan(self):
        try:
            async with async_playwright() as p:
                ...
                if not gpt or not gpt.is_active():
                    self.voice_available = False
                    self.voice_enabled = False
                    self.voice_gpt_url = None
            ...
            async with async_playwright() as p:
                ...
                for node in self.relay.active_nodes:
                    ...
                    if responded:
                        ...
                        if clean and not dropped:
                            if node.name == "GPT":
                                await self._update_gpt_voice_capability(
                                    page, node, preserve_enabled=True
                                )
    ...
    async def _find_read_aloud_selector(self, page, node):
        """Find GPT Read aloud, including the current overflow-menu UI."""
        if node.name != "GPT":
            return None

        # First reveal the newest response toolbar. Some ChatGPT builds expose
        # Read aloud directly; newer builds may place it under More actions.
        try:
            await page.evaluate("""
                (responseSelector) => {
                    function visible(el) {
                    ...
                    let responses = [];
                    try { responses = [...document.querySelectorAll(responseSelector)]; } catch (e) { return false; }
                    ...
                    try {
                        response.dispatchEvent(new MouseEvent('mouseenter', {bubbles:true}));
                        response.dispatchEvent(new MouseEvent('mouseover', {bubbles:true}));
                    } catch (e) {}
                    return true;
                }
        ...
        # Direct/native Read aloud controls first.
        direct_selector = await page.evaluate("""
            () => {
                function visible(el) {
                    if (!el) return false;
                    const s = getComputedStyle(el);
                ...
                for (const sel of selectors) {
                    const matches = [...document.querySelectorAll(sel)].filter(visible);
                    if (matches.length) return sel;
                }
                return null;
            }
        """)
        ...
        # Current ChatGPT web UI may put Read aloud inside the response's
        # More actions overflow menu. Use the actual browser click so Radix/UI
        # menu event handling receives a real interaction sequence.
        try:
            more_selector = await page.evaluate("""
                (responseSelector) => {
                    function visible(el) {
                        if (!el) return false;
                        const s = getComputedStyle(el);
                    ...
                    let responses = [];
                    try { responses = [...document.querySelectorAll(responseSelector)]; } catch (e) {}
                    responses = responses.filter(el => visible(el) && (el.innerText || el.textContent || '').trim().length > 0);
                    const newest = responses[responses.length - 1];
                    if (!newest) return null;

                    const candidates = [
                        ...newest.querySelectorAll('button, [role="button"]')
                    ].filter(el => {
                        if (!visible(el)) return false;
                        const h = label(el);
                        return h.includes('more actions') ||
                               h.includes('more options') ||
                               h === 'more' ||
                               h.includes('show more');
                    });
                    const scoped = candidates[candidates.length - 1];
                    if (scoped) return uniqueSelector(scoped);
                    return null;
                }
            """, node.output_selector)

            if more_selector:
                more_button = await page.query_selector(more_selector)
                if more_button and await more_button.is_visible():
                    await more_button.click()
                    await asyncio.sleep(0.35)

                    menu_selector = await page.evaluate("""
                        () => {
                            function visible(el) {
                                if (!el) return false;
                                const s = getComputedStyle(el);
                            ...
                            function wanted(el) {
                                ...
                                return haystack.includes('read aloud') ||
                                       haystack.includes('readaloud') ||
                                       haystack.includes('朗读');
                            }
                            function uniqueSelector(el) {
                                if (el.id) {
                                    const id = '#' + CSS.escape(el.id);
                            ...
                            return menuItems.length ? uniqueSelector(menuItems[menuItems.length - 1]) : null;
                        }
                    """)
                    if menu_selector:
    ...
    async def _dismiss_read_aloud_menu(self, page):
        try:
            await page.keyboard.press("Escape")
        except Exception:
            pass

    async def _update_gpt_voice_capability(self, page, node, preserve_enabled=False):
        """Probe live GPT Read aloud capability for the newest normal response.

        Voice is a simple capability/setting pair rather than a second relay
        state machine: unavailable, or available with ON/OFF.  Capability is
        established from an actual completed response, not from handshake UI.
        """
        ...
        self.ui.log(
            "[VOICE] GPT response completed; checking for Read aloud control.",
            is_system=True
        )
        ...
        for _ in range(5):
            try:
                selector = await self._find_read_aloud_selector(page, node)
            except Exception as exc:
                self.ui.log(
                    f"[VOICE] Read aloud probe failed: {exc}",
                    is_system=True
                )
                selector = None
            if selector:
        ...
        # Capability probing may have opened GPT's More actions menu. Close it
        # after a probe so an OFF voice setting does not leave the UI obstructed.
        if selector:
            await self._dismiss_read_aloud_menu(page)
        ...
        if available:
            self.ui.log(
                "[VOICE] GPT Read aloud control FOUND. Voice toggle available.",
                is_system=True
            )
            if self.voice_enabled:
                self.ui.log(
                    "[VOICE] Voice remains ON; capability re-verified.",
                    is_system=True
                )
            else:
                self.ui.log(
                    "[VOICE] Voice is OFF until Cozmo enables it.",
                    is_system=True
                )
        else:
            self.ui.log(
                "[VOICE] GPT Read aloud control NOT FOUND. Voice toggle remains disabled.",
                is_system=True
            )
    ...
    def toggle_voice(self):
        ...
        if not gpt or not gpt.is_active() or not self.voice_available:
            ...
            self.ui.set_voice_state(False, False)
            self.ui.log(
                "[VOICE] Toggle ignored: active GPT Read aloud capability has not been verified.",
                is_system=True
            )
            return
    ...
    async def _read_aloud_latest_response(self, page, node):
        """Best-effort click of GPT's Read aloud control for the newest response."""
        if node.name != "GPT":
            return False

        try:
            self.ui.log(
                "[READ ALOUD] GPT: preparing newest response for voice.",
                is_system=True
            )
            for attempt in range(5):
                ...
                if selector:
                    ...
                    if button and await button.is_visible():
                        ...
                        await self._dismiss_read_aloud_menu(page)
                        self.ui.log(
                            f"[READ ALOUD] GPT: clicked Read aloud on newest response (attempt {attempt + 1}).",
                            is_system=True
                        )
                        return True
            ...
            self.ui.log(
                "[READ ALOUD] GPT: Read aloud control not found after retries.",
                is_system=True
            )
            return False
        except Exception as exc:
            self.ui.log(
                f"[READ ALOUD] GPT: click failed (relay continues): {exc}",
                is_system=True
            )
            return False
