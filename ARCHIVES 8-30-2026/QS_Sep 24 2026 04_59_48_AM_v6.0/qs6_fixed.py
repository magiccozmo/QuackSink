...
# VERSION: QS 6.0 - standalone candidate v6
# QS 5.11 DEVELOPMENT:
...
#   - This preserves the known-good QS 5.9 handshake state machine while making
...
# Retained for historical compatibility/documentation; QS6 no longer sends a
# second token-framed handshake after the Library preamble.
...
class Node:
    def __init__(self, name, url, icon="🔮"):
        ...
        self.handshake_baseline_signature = None
        self.handshake_user_baseline_count = 0
        self.handshake_user_baseline_match_count = 0
        self._listeners = []
        self._var = None
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        # Utility buttons. The obsolete archive-directory selector is gone;
        # clipboard actions operate on the current active archive automatically.
        utility_frame = tk.Frame(self.control_frame, bg="#121212")
        utility_frame.pack(fill=tk.X, padx=12, pady=(6, 2))

        utility_specs = [
            ("COPY LOG TO CLIPBOARD AS TEXT", self._copy_active_log, "#0066AA", 8),
            ("COPY SOURCE TO CLIPBOARD AS TEXT", self._copy_active_source, "#663399", 8),
            ("LIBRARY", self._open_library, "#0088AA", 10),
            ("WHAT CAN THIS DO?", self._open_what_can_this_do, "#AA5500", 10),
        ]
        for label, command, bg, padx in utility_specs:
            tk.Button(
                utility_frame,
                text=label,
                command=command,
        ...
        self.root.after_idle(self._fit_control_pane)

    def _fit_control_pane(self):
        """Size the control pane to its actual contents so COMMS gets the rest."""
        if getattr(self, "_comms_maximized", False):
            return
        try:
            self.root.update_idletasks()
            requested = self.control_frame.winfo_reqheight()
            pane_height = self.pane.winfo_height()
            if pane_height <= 0:
                return
            # Keep a small visual margin, but do not reserve a fossil 520px block.
            max_sash = max(260, pane_height - 300)
            sash_y = min(max(requested + 6, 260), max_sash)
            self.pane.sash_place(0, 0, sash_y)
        except Exception:
            pass
    ...
    def _make_title_file(self):
        ...
        archive_path = ACTIVE_ARCHIVE_DIR
    ...
    def _copy_active_log(self):
        log_path = os.path.join(ACTIVE_ARCHIVE_DIR, "LOG.txt")
    ...
    def _copy_active_source(self):
        source_path = os.path.join(ACTIVE_ARCHIVE_DIR, RUNTIME_FILE)
...
class QSController:
    ...
    async def _click_gpt_read_aloud(self, page):
        ...
        QS 5.9 hardening:
        - waits briefly for the More Actions menu to finish rendering
        - searches only visible/open menu containers first
        - falls back to an exact global semantic match only if needed
        """
        try:
            result = await page.evaluate(r"""
                (() => {
                    ...
                    moreTarget.click();
                    return {ok:true, label:clean(moreTarget.getAttribute('aria-label') || moreTarget.textContent || '')};
                })()
            """)
        except Exception as exc:
        ...
        # ChatGPT's menu can render a fraction later than the click that opens it.
        # Wait briefly for the actual semantic Read aloud item instead of probing once.
                ...
                menu_item = await page.evaluate(r"""
                    (() => {
                        ...
                        for (const menu of menus.reverse()) {
                            const match = Array.from(menu.querySelectorAll('[role="menuitem"],button,[role="button"],[data-testid]'))
                                .find(exact);
        ...
        # One final exact semantic fallback.
        try:
            menu_item = await page.evaluate(r"""
                (() => {
                    const matches = Array.from(document.querySelectorAll('[role="menuitem"],button,[role="button"],[data-testid]')).filter((el) => {
                        const style = getComputedStyle(el);
                        const rect = el.getBoundingClientRect();
                        ...
                        return /^read\s+aloud$/i.test(label) || /^read[-_\s]+aloud(?:[-_\s]+button)?$/i.test(label);
                    });
            ...
            """)
            if menu_item:
                self.ui.log("[VOICE] GPT Read aloud menu item clicked (semantic fallback).", is_system=True)
                return True
        except Exception:
            pass
    ...
    async def _gpt_voice_stop_visible(self, page):
        """Return True only when the latest GPT response itself exposes playback.

        Do not use global page-wide Stop/scroll/menu labels: GPT can leave menus or
        unrelated controls visible while speech is already finished.
        """
        try:
            return bool(await page.evaluate(r"""
                (() => {
                    ...
                    // Only inspect the latest response and its immediate action containers.
                    const scopes = [latest];
                    ...
                    const isPlayback = (el) => {
                        ...
                        // Explicit playback labels only. Do not match bare Stop/scroll controls.
                        return /^(?:stop(?:\s+reading(?:\s+aloud)?)?|reading\s+aloud|speaking|pause(?:\s+reading(?:\s+aloud)?)?)$/i.test(label) ||
                               /\b(?:stop|pause)\s+(?:reading\s+aloud|speech|speaking|playback)\b/i.test(label);
                    };

                    for (const scope of scopes) {
                        const controls = Array.from(scope.querySelectorAll('button,[role="button"],[data-testid],[aria-label],[title]'))
                            .filter((el) => !el.closest('[role="menu"],[role="menuitem"],nav,header'));
                        if (controls.some(isPlayback)) return true;
                    }

                    // A real active audio element or speechSynthesis signal is stronger evidence
                    // than a generic button somewhere on the page.
                    const activeAudio = Array.from(document.querySelectorAll('audio')).some((audio) => {
                        try { return !audio.paused && audio.readyState >= 2; } catch (_) { return false; }
                    });
    ...
    async def _send_message_to_page(self, page, node, text, is_handshake):
        ...
        try:
            ...
            if is_handshake:
                node.handshake_baseline_signature = await self._capture_latest_assistant_signature(page, node)
                node.handshake_user_baseline_count = await self._capture_user_message_count(page, node)
                node.handshake_user_baseline_match_count = await self._capture_submitted_text_match_count(
                    page, node, node.last_submitted_text
                )
                self.ui.log(
                    f"[HANDSHAKE-BASELINE] {node.name}: assistant_signature_set="
                    f"{bool(node.handshake_baseline_signature)} "
                    f"user_message_count={node.handshake_user_baseline_count} "
                    f"submitted_text_matches={node.handshake_user_baseline_match_count}",
                    is_system=True
                )
            step = "SUBMIT"
    ...
    async def _submit_verified(self, page, node, input_element, selector, method, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start < timeout:
            ...
            if is_handshake:
                new_user_turn = await self._handshake_user_turn_detected(page, node)
                # IMPORTANT: a generic output change is NOT submission evidence for
                # the handshake. The browser may expose stale/previous assistant
                # content through a broad selector. Response detection happens only
                # after the input->submitted transition is independently proven.
                submitted = new_user_turn or ((not still_has_text) and busy)
                if submitted:
                    reason = "new_user_turn" if new_user_turn else "composer_clear+busy"
                    self.ui.log(
                        f"[SEND] SUBMIT CONFIRMED ({method}): {node.name} | "
                        f"input_clear={not still_has_text} busy={busy} "
                        f"new_output={new_output} new_user_turn={new_user_turn} "
                        f"handshake=True evidence={reason}",
                        is_system=True
                    )
                    return True
            elif method in ("Enter", "Enter/CDP"):
                submitted = new_output or ((not still_has_text) and busy)
                if submitted:
                    reason = "new_output" if new_output else "composer_clear+busy"
                    self.ui.log(
                        f"[SEND] SUBMIT CONFIRMED ({method}): {node.name} | "
                        f"input_clear={not still_has_text} busy={busy} "
    ...
    async def _submit(self, page, node, input_element, selector, is_handshake=False):
        ...
        # Handshake is a state-machine boundary.  Prefer the real Send control
        # because the browser has already demonstrated that it can submit the
        # preamble through this exact UI path.  Enter remains a fallback, but a
        # cleared composer by itself is NEVER enough to declare the handshake sent.
        if is_handshake:
            send_selectors = [
            ...
            for attempt in range(10):
                    ...
                    try:
                        ...
                        for button in buttons:
                            ...
                            self.ui.log(
                                ...
                                f"using '{send_selector}' attempt={attempt + 1}/10",
                                is_system=True
                            )
                            await button.click()
                ...
                if attempt < 9:
                    self.ui.log(
                        f"[SEND] HANDSHAKE BUTTON WAIT: {node.name} "
                        f"attempt={attempt + 1}/10 candidate={found_candidate}",
                        is_system=True
                    )
                    await asyncio.sleep(0.20)

            # Enter fallback: refocus the current composer immediately before each
            # attempt.  The submission verifier now requires genuine evidence that
            # the user turn left the input; composer_clear alone is insufficient.
            ...
            # Native target-level key event fallback.
            for enter_attempt in range(2):
                try:
                    live_input = await self._refocus_live_input(page, selector)
                    if live_input is None:
            ...
            # Final synthetic DOM Enter fallback.
            try:
                live_input = await self._refocus_live_input(page, selector)
                if live_input is None:
                    raise RuntimeError("current composer could not be focused")
    ...
    async def _capture_latest_assistant_signature(self, page, node):
        """Return an identity/content signature for the latest assistant turn.

        This is a pre-submit baseline. A later handshake frame is accepted only
        when the latest assistant turn is observably different from this baseline.
        That prevents an old 🟩...🔚 frame inside a historical DOM node from being
        mistaken for the new handshake response.
        """
        try:
            ...
            result = await page.evaluate(rf"""
                (() => {{
                    ...
                    let elements = [];
                    const addUnique = (el) => {{
                        if (el && !elements.includes(el) && visible(el) &&
                    ...
                    try {{
                        ...
                        if (assistants.length) assistants.forEach(addUnique);
                    }} catch (_) {{}}

                    if (!elements.length) {{
                        try {{ document.querySelectorAll(outputSelector).forEach(addUnique); }} catch (_) {{}}
                    }}

                    if (!elements.length) {{
                        for (const sel of [
                            '[data-message-id]', '[data-testid*="message"]',
                            '[role="article"]', 'article', 'div[class*="message" i]'
                        ]) {{
                            try {{ document.querySelectorAll(sel).forEach(addUnique); }} catch (_) {{}}
                        }}
                    ...
                    return String(elements.length) + '\u001f' + messageId + '\u001f' + text;
                }})()
            """)
            return str(result) if result else None
        except Exception as exc:
            self.ui.log(
                f"[WAIT-DIAG] {node.name}: assistant baseline capture failed: "
                f"{type(exc).__name__}: {exc}",
                is_system=True
            )
            return None

    async def _capture_user_message_count(self, page, node):
        """Return a conservative count of visible user-turn containers.

        Chat UIs do not all expose the same user-message attributes. Prefer the
        explicit role selectors, then fall back to conversation-turn/article
        containers whose exact text can still be checked by the submission probe.
        """
        try:
            result = await page.evaluate("""
                () => {
            ...
            self.ui.log(
                f"[WAIT-DIAG] {node.name}: user-message baseline capture failed: "
                f"{type(exc).__name__}: {exc}",
                is_system=True
            )
            return 0

    async def _capture_submitted_text_match_count(self, page, node, text):
        """Count visible non-composer DOM containers whose canonical text exactly matches text.

        Gemini does not currently expose a dependable user-message role selector in the
        inspected DOM. We therefore track the number of exact submitted-text turn matches
        before submission and require that number to increase afterward. This avoids both
        the old false negatives (no user-role selector) and false positives from the
        composer itself.
        """
        target = self._canonical_message_text(text)
        if not target:
            return 0
        try:
                        ...
                        try {{
                            for (const el of document.querySelectorAll(selector)) {{
                                ...
                                if (el.matches('textarea, input, [contenteditable="true"]') ||
                                    el.closest('textarea, input, [contenteditable="true"]')) continue;
                                const value = canonical(el.innerText || el.textContent || '');
                                if (value === target) matches += 1;
                            }}
                        }} catch (_) {{}}
                    }}
                    return matches;
            ...
            self.ui.log(
                f"[WAIT-DIAG] {node.name}: submitted-text match probe failed: "
                f"{type(exc).__name__}: {exc}",
                is_system=True
            )
            return 0

    async def _handshake_user_turn_detected(self, page, node):
        """Prove the submitted handshake created a new visible user turn.

        Prefer explicit user-role selectors, but fall back to an exact submitted-text
        match-count increase. The fallback is deliberately baseline-relative so an
        older identical handshake in chat history cannot satisfy a fresh submission.
        """
        submitted = self._canonical_message_text(node.last_submitted_text)
        if not submitted:
            return False
        try:
            safe_submitted = json.dumps(submitted)
            baseline_count = int(node.handshake_user_baseline_count or 0)
            baseline_match_count = int(node.handshake_user_baseline_match_count or 0)
            safe_baseline_count = json.dumps(baseline_count)
            safe_baseline_match_count = json.dumps(baseline_match_count)
            result = await page.evaluate(rf"""
                (() => {{
                    const target = {safe_submitted};
                    const baselineCount = {safe_baseline_count};
                    const baselineMatchCount = {safe_baseline_match_count};
                    const visible = (el) => {{
                        if (!el) return false;
                        const s = getComputedStyle(el);
                        const r = el.getBoundingClientRect();
                    ...
                    for (const selector of selectors) {{
                        try {{
                            for (const el of document.querySelectorAll(selector)) {{
                                if (seen.has(el) || !visible(el)) continue;
                                seen.add(el);
                                if (el.matches('textarea, input, [contenteditable="true"]')) continue;
                                const text = canonical(el.innerText || el.textContent || '');
                                if (text) texts.push(text);
                            }}
                        }} catch (_) {{}}
                    }}
                    const currentCount = texts.length;
                    const explicitMatched = currentCount > baselineCount &&
                        texts.slice(baselineCount).some((text) => text === target);
                    return {{currentCount, explicitMatched}};
                }})()
            """)
            explicit_matched = bool(
                result and result.get('explicitMatched') and
                int(result.get('currentCount', 0)) > baseline_count
            )
            if explicit_matched:
                return True

            current_match_count = await self._capture_submitted_text_match_count(
                page, node, submitted
            )
            fallback_matched = current_match_count > baseline_match_count
            if fallback_matched:
                self.ui.log(
                    f"[WAIT-DIAG] {node.name}: handshake user-turn fallback matched exact "
                    f"submitted text ({baseline_match_count} -> {current_match_count}).",
                    is_system=True
                )
            return fallback_matched
        except Exception as exc:
            self.ui.log(
                f"[WAIT-DIAG] {node.name}: handshake user-turn probe failed: "
                f"{type(exc).__name__}: {exc}",
                is_system=True
            )
            return False
    ...
    async def _capture_latest_handshake_frame(self, page, node):
        ...
        try:
            ...
            safe_baseline_signature = json.dumps(node.handshake_baseline_signature)
            result = await page.evaluate(rf"""
                (() => {{
                    ...
                    const signature = String(elements.length) + '\u001f' + messageId + '\u001f' + text;
                    const baselineSignature = {safe_baseline_signature};
                    if (!baselineSignature) {{
                        return {{ok:false, reason:'no pre-submit assistant baseline available'}};
                    }}
                    if (signature === baselineSignature) {{
                        return {{ok:false, reason:'latest assistant turn is unchanged from pre-submit baseline'}};
                    }}
                    const matches = Array.from(text.matchAll(/🟩[\s\S]*?🔚/g));
                    if (!matches.length) {{
                        return {{ok:false, reason:'latest assistant turn changed, but contains no complete handshake frame'}};
                    }}
    ...
    async def _wait_for_response(self, page, node, timeout=45, is_handshake=False):
        ...
        while True:
            ...
            current_response_text = None if is_handshake else (str(candidates[-1]).strip() if candidates else None)
            ...
            if is_handshake:
                # HANDSHAKING is a state-machine contract, not a generic response
                # extraction problem. Read the latest assistant turn directly.
                # The generic candidate list can mix parent/child message wrappers
                # (especially on GPT), causing a fresh handshake frame to be
                # mistaken for stale or nested output.
                handshake_frame = await self._capture_latest_handshake_frame(page, node)
