...
class SelectorDetector:
    ...
    def get_known_selectors(name):
        known = {
            ...
            "Claude": ("textarea, [contenteditable='true']", "[data-testid=\"assistant-message\"], .font-claude-response, .font-claude-message, [data-testid=\"message-assistant\"], .assistant-message"),
...
class QSController:
    ...
    async def _submit_verified(self, page, node, input_element, selector, method, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start < timeout:
            ...
            if is_handshake:
                ...
                submitted = new_output or new_user_turn or ((not still_has_text) and busy)
                if submitted:
                    if new_output:
                        reason = "new_output"
                    elif new_user_turn:
    ...
    async def _capture_claude_latest(self, page, node):
        """Capture Claude assistant messages using a resilient selector chain.

        Claude's web UI has changed message-container class names over time.
        Keep several known assistant selectors as fallbacks, but use ONE selector
        at a time so overlapping parent/child matches cannot double-count a turn.
        """
        selectors = [
        ...
        # Prefer the selector already stored by discovery when it is one of the
        # known Claude selectors. Otherwise walk the known fallbacks.
        ordered = []
        if node.output_selector:
            ordered.append(node.output_selector)
        ...
        for selector in ordered:
            ...
            if isinstance(value, list) and any(str(x).strip() for x in value):
                if selector != node.output_selector:
                    self.ui.log(
                        f"[CLAUDE-CAPTURE] Output selector auto-repaired: {node.output_selector!r} -> {selector!r}",
                        is_system=True
                    )
                    node.output_selector = selector
                return value
    ...
    async def _capture_user_message_count(self, page, node):
        """Return the visible user-message count used to prove a handshake was sent."""
        try:
            result = await page.evaluate("""
                () => {
                    const visible = (el) => {
                    ...
                    for (const selector of sets) {{
                        try {{
                            for (const el of document.querySelectorAll(selector)) {{
                                ...
                                seen.add(el);
                                texts.push(canonical(el.innerText || el.textContent || ''));
                            }}
                        }} catch (_) {{}}
                    }}
                    const currentCount = texts.length;
                    const matched = texts.slice(baseline_count).some((text) => text === target);
                    return {{currentCount, matched}};
                }})()
            """)
            return bool(result and result.get('matched') and int(result.get('currentCount', 0)) > baseline_count)
        except Exception as exc:
            self.ui.log(
