...
RUNTIME_FILE = "qs6_fixed_v6_9.py"
...
class QSController:
    ...
    async def _click_gpt_read_aloud(self, page, response_text=None):
        """GPT: click Read aloud for the exact response QS just captured.

        6.7 keeps the response-text binding from 6.6, but resolves the current
        ChatGPT action row through the enclosing conversation turn. ChatGPT's
        current action control may be exposed as simply aria-label="More", not
        literally "More Actions". No page-wide More-button fallback is allowed.
        """
        try:
            ...
            result = await page.evaluate(rf"""
                (() => {{
                    ...
                    // Prefer the current conversation-turn wrapper: current ChatGPT
                    // action buttons live in the turn's action row, often as a sibling
                    // of the assistant message rather than a descendant of it.
                    let turn = latest.closest('[data-testid^="conversation-turn-"]');
                    if (!turn) {{
                        let probe = latest.parentElement;
                        for (let depth = 0; depth < 10 && probe; depth += 1) {{
                            if (probe.querySelector && probe.querySelector('[data-message-author-role="assistant"]')) {{
                                turn = probe;
                                break;
                            }}
                    ...
                    for (const scope of scopes) {{
                        ...
                        )).filter((el) => visible(el) && !el.closest('[role="menu"],[role="menuitem"],nav,header'));
                        const semantic = candidates.filter(isMore);
                        if (semantic.length) {{
                            moreTarget = semantic[semantic.length - 1];
                            break;
                        }}
                    }}

                    if (!moreTarget) {{
                        const localButtons = turn ? Array.from(turn.querySelectorAll('button,[role="button"],[data-testid]'))
                            .filter((el) => visible(el)) : [];
                        const diagnostics = localButtons.map((el) => labels(el).join(' | ')).filter(Boolean).slice(-30);
                        return {{ok:false, stage:'more_actions', reason:'GPT More control not found in exact conversation turn/action scope.', diagnostics}};
                    }}

                    moreTarget.click();
                    return {{
                        ok:true,
                        responseMatched:Boolean(exact.length),
                        controlLabel:labels(moreTarget).join(' | '),
                        turnBound:Boolean(turn)
                    }};
                }})()
            """)
        except Exception as exc:
        ...
        if not result or not result.get('ok'):
            ...
            self.ui.log(f"[VOICE] GPT More Actions unavailable: {reason}", is_system=True)
            if result and result.get('diagnostics'):
                self.ui.log(
                    f"[VOICE] GPT local action controls: {result['diagnostics']}",
                    is_system=True
                )
            return False
        ...
        self.ui.log(
            ...
            f"turn_bound={result.get('turnBound')}, label={result.get('controlLabel')!r}).",
            is_system=True
        )
    ...
    async def _gpt_voice_state(self, page, response_text=None):
        """Return GPT native Read Aloud state for the exact captured response."""
        try:
            safe_response = json.dumps(self._canonical_message_text(response_text or ""))
            result = await page.evaluate(rf"""
                (() => {{
                    ...
                    const controls = [];
                    for (const scope of scopes) {{
                        for (const el of scope.querySelectorAll('button,[role="button"],[data-testid],[aria-label],[title]')) {{
                            if (!visible(el)) continue;
                    ...
                    if (active) return {{state:'active',label:labels(active).join(' | '),responseMatched:Boolean(matches.length)}};

                    const idle = controls.find((el) => labels(el).some((value) =>
                        /^read\s+aloud$/i.test(value) ||
                        /^read[-_\s]+aloud(?:[-_\s]+button)?$/i.test(value)
                    ));
                    if (idle) return {{state:'idle',label:labels(idle).join(' | '),responseMatched:Boolean(matches.length)}};

                    return {{state:'unknown', reason:'No response-scoped GPT Read Aloud state detected', responseMatched:Boolean(matches.length)}};
                }})()
            """)
    ...
    async def _wait_for_voice_playback(self, page, node, response_text=None):
        """Wait for native playback without allowing GPT UNKNOWN state to deadlock QS.

        ACTIVE  = positive response-scoped playback evidence.
        IDLE    = positive response-scoped completion evidence after ACTIVE.
        UNKNOWN = Read aloud launched, but the browser exposes no trustworthy
                  response-scoped playback state.

        UNKNOWN is never relabeled as proven COMPLETE.  For GPT it has a
        conservative, response-length-aware fallback so the relay cannot remain
        blocked forever.  Claude retains the QS 6.7 blocking behavior.
        """
        start = asyncio.get_event_loop().time()
        saw_playback = False
        idle_streak = 0
        ...
        # ------------------------------------------------------------------
        # GPT: bounded UNKNOWN path.
        # ------------------------------------------------------------------
        if node.name == 'GPT':
            words = len(re.findall(r"\S+", response_text or ""))

            # Approximate spoken duration at ~120 words/minute.  Keep a minimum
            # grace period so short responses do not release immediately, and
            # cap the estimate so a huge reply cannot stall QS for ages.
            estimated_speech = max(8.0, min(180.0, words / 2.0))
            fallback_after = max(20.0, estimated_speech + 8.0)

            # Give GPT up to the normal 30 seconds to expose ACTIVE, but never
            # let the UNKNOWN state extend beyond the bounded fallback window.
            startup_deadline = min(30.0, fallback_after)
            ...
            while elapsed() < startup_deadline:
                ...
                if state.get('state') == 'active':
                    saw_playback = True
                    self.ui.log(
                        f"[VOICE] GPT playback started; "
                        f"state={state.get('label', 'active')!r}; "
                        "waiting for native playback to finish.",
                        is_system=True
                    )
                    break
            ...
            if not saw_playback:
                self.ui.log(
                    f"[VOICE] GPT native playback state remains UNKNOWN after "
                    f"{elapsed():.1f}s; bounded release window is "
                    f"{fallback_after:.1f}s.",
                    is_system=True
                )

                # Keep watching for a late ACTIVE signal during the bounded
                # window.  If it appears, enter the normal ACTIVE -> IDLE path.
                while elapsed() < fallback_after:
                    state = await self._gpt_voice_state(
                        page,
                        response_text=response_text
                    )
                    ...
                    if state.get('state') == 'active':
                        saw_playback = True
                        self.ui.log(
                            "[VOICE] GPT playback state became observable late; "
                            f"state={state.get('label', 'active')!r}.",
                            is_system=True
                        )
                        break
            ...
            if not saw_playback:
                self.ui.log(
                    "[VOICE] GPT playback completion remained unobservable; "
                    f"releasing relay after bounded UNKNOWN window "
                    f"({elapsed():.1f}s).",
                    is_system=True
                )
                return False

            # ACTIVE was observed.  Require two consecutive IDLE observations
            # so an idle reading sampled too early cannot end speech.
            last_active = elapsed()
            idle_streak = 0
            ...
            while elapsed() < 1800.0:
                ...
                if current == 'active':
                    ...
                    if idle_streak >= 2:
                        self.ui.log(
                            "[VOICE] GPT playback complete; exact response voice "
                            "control returned to idle.",
                            is_system=True
                        )
                        return True
                else:
                    ...
                    # Do not recreate the 6.7 deadlock if playback telemetry
                    # disappears after ACTIVE.  Once the conservative speech
                    # window has elapsed, release with UNKNOWN explicitly logged.
                    if elapsed() >= fallback_after and elapsed() - last_active >= 8.0:
                        self.ui.log(
                            "[VOICE] GPT playback state became UNKNOWN after ACTIVE; "
                            f"releasing relay after bounded safety window "
                            f"({elapsed():.1f}s).",
                            is_system=True
                        )
                        return False
            ...
            self.ui.log(
                "[VOICE] GPT native playback safety timeout reached; releasing relay.",
                is_system=True
            )
            return False

        # ------------------------------------------------------------------
        # Claude: QS 6.7 behavior retained.
        # ------------------------------------------------------------------
        while elapsed() < 30.0:
            if node.name == 'Claude':
                state = await self._claude_voice_state(
    ...
    async def _maybe_speak_node(self, page, node, response_text):
        ...
        if node.name == 'GPT' and not completed:
            self.ui.log(
                "[VOICE] GPT native playback completion not proven; "
                "bounded voice wait released the relay turn. No retry.",
                is_system=True
            )
            return
