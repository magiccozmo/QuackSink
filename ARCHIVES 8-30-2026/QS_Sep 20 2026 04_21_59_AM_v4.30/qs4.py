...
class QSUI:
    ...
    def _toggle_node_voice(self, name):
        ...
        if name not in VOICE_CAPABLE_NODES:
            ...
            self.log(f"[VOICE] {name} voice is not available in QS 4.30.", is_system=True)
            return
...
class QSController:
            ...
            opened = await page.evaluate(r"""
                (() => {
                    ...
                    const isMore = (el) => {
                        ...
                        return /more/i.test(label) || /^(?:\\.\\.\\.|…|⋯)$/.test(label);
                    };
    ...
    async def _maybe_speak_node(self, page, node):
        ...
        if node.name == 'GPT':
            ...
            self.ui.log(f"[VOICE] {node.name} voice path is unavailable in QS 4.30.", is_system=True)
            return
