...
class QSController:
                ...
                if success:
                    ...
                    if responded:
                        ...
                        if clean and not dropped:
                            ...
                            if node.name == "GPT":
                                await self._update_gpt_voice_capability(page, node)
            ...
            async with async_playwright() as p:
                ...
                for node in self.relay.active_nodes:
                    ...
                    if responded:
                        ...
                        if clean and not dropped:
                            if node.name == "GPT" and self.voice_enabled and self.voice_available:
    ...
    async def _update_gpt_voice_capability(self, page, node):
        ...
        self.ui.log(
            "[VOICE] GPT is connected; checking for Read aloud control.",
            is_system=True
        )
        selector = None
        for _ in range(5):
            try:
                ...
                self.ui.log(f"[VOICE] Read aloud probe failed: {exc}", is_system=True)
                selector = None
            if selector:
                break
        ...
        self.voice_enabled = False
        self.ui.set_voice_state(available, False)
        if available:
            self.ui.log(
                "[VOICE] GPT Read aloud control FOUND. Voice toggle enabled (starts OFF).",
                is_system=True
            )
        else:
            self.ui.log(
    ...
    def toggle_voice(self):
        if not self.voice_available:
            self.ui.log(
                "[VOICE] Toggle ignored: GPT Read aloud capability is not currently verified.",
                is_system=True
            )
            return
