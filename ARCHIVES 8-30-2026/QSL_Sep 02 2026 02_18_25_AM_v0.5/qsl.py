...
def wait_for_browser_ready(
    ...
    while time.monotonic() < deadline:
        ...
        if not cdp_is_available():
            append_log(
                archive,
                "Readiness failed: CDP endpoint disappeared."
            )
            return False
        ...
        append_log(
            ...
            f"stable={stable}/{BROWSER_STABLE_POLLS_REQUIRED}."
        )
...
def launch_browser_via_bat(
    ...
    append_log(
        archive,
        f"Launching browser BAT through "
        f"{command_com}: {bat}"
    )
...
def prepare_browser(
    ...
    if cdp_is_available():
        ...
        append_log(
            archive,
            "CDP already available before browser launch; "
            "existing browser path accepted."
        )
    ...
    try:
        ...
        append_log(
            archive,
            f"Browser BAT handed to COMMAND.COM. "
            f"PID={proc.pid}."
        )
        ...
        append_log(
            archive,
            f"Browser BAT launch FAILED: {exc}"
        )

        messagebox.showerror(
            "Browser Launch Failed",
            "QSL could not start the browser launch BAT "
            "through COMMAND.COM.\n\n"
            f"{bat}\n\n"
            f"Error:\n{exc}"
        )
    ...
    # --------------------------------------------------------------
    # IMPORTANT:
    # These are the existing post-launch readiness checks.
    # Do not launch QuackSink until BOTH have succeeded.
    # --------------------------------------------------------------
...
def wait_for_cdp(
    ...
    while time.monotonic() < deadline:
        ...
        if cdp_is_available():
            ...
            append_log(
                archive,
                f"CDP became available on port {CDP_PORT}."
            )
    ...
    append_log(
        archive,
        f"CDP did not become available within "
        f"{BROWSER_READY_TIMEOUT:.1f}s."
    )

    messagebox.showwarning(
        "Remote Debugging Unavailable",
        "The selected browser was started, but QSL "
        f"could not detect remote debugging on port {CDP_PORT}.\n\n"
        "QSL will not launch QuackSink until the browser "
        "is reachable through CDP."
    )
...
class QuackSpaceLauncher:
    ...
    def launch_quacksink(self):
        ...
        # BROWSER STARTUP
        #
        # If CDP already exists, leave the existing browser alone.
        #
        # If CDP does not exist:
        #   1. Create openbrowser.bat
        #   2. Run it through COMMAND.COM
        #   3. Wait for CDP
        #   4. Wait for browser/page stability
        #   5. ONLY THEN continue to QuackSink
        # ----------------------------------------------------------
        ...
        if not cdp_is_available():
            ...
            self.status_var.set(
                f"Starting {name} through browser BAT...\n"
                "Waiting for browser/page readiness "
                "before launching QuackSink."
            )
            ...
            append_log(
                self.active_archive,
                "Existing CDP detected; "
                "browser startup/readiness wait skipped."
            )
