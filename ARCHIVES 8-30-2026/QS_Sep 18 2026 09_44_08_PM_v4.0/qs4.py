...
# QS 4.0 browser transport: Puppeteer via a persistent Node bridge.
...
async function findPage(urlHint) {
...
async function handle(cmd) {
    switch (cmd.op) {
        ...
        case 'title': {
            const page = await findPage(cmd.url);
        ...
        case 'element_is_visible': {
            const page = await findPage(cmd.url);
            if (!page) throw new Error('Page not found: ' + cmd.url);
            const locator = page.locator(cmd.selector).nth(cmd.index || 0);
        ...
        case 'element_get_attribute': {
            ...
            if (!page) throw new Error('Page not found: ' + cmd.url);
            const locator = page.locator(cmd.selector).nth(cmd.index || 0);
            return await locator.getAttribute(cmd.attribute);
        }
...
class PuppeteerElement:
    def __init__(self, bridge, url, selector, index=0):
        self.bridge = bridge
        self.url = url
    ...
    async def is_visible(self):
        return bool(await asyncio.to_thread(
            self.bridge.request, {
                "op": "element_is_visible", "url": self.url,
    ...
    async def is_disabled(self):
        return bool(await asyncio.to_thread(
            self.bridge.request, {
                "op": "element_is_disabled", "url": self.url,
                "selector": self.selector, "index": self.index
            }
        ))
    ...
    async def get_attribute(self, attribute):
        return await asyncio.to_thread(
            self.bridge.request, {
                "op": "element_get_attribute", "url": self.url,
                "selector": self.selector, "index": self.index,
                "attribute": attribute
            }
        )
    ...
    async def inner_text(self):
        return await asyncio.to_thread(
            self.bridge.request, {
                "op": "element_inner_text", "url": self.url,
                "selector": self.selector, "index": self.index
            }
        )
    ...
    async def input_value(self):
        return await asyncio.to_thread(
            self.bridge.request, {
                "op": "element_input_value", "url": self.url,
                "selector": self.selector, "index": self.index
            }
        )
...
class PuppeteerKeyboard:
    ...
    async def press(self, key):
        return await asyncio.to_thread(
            self.bridge.request, {
                "op": "keyboard_press", "url": self.url, "key": key
            }
        )


class PuppeteerPage:
    def __init__(self, bridge, url, title="(no title)"):
        self.bridge = bridge
        self.url = url
        self._title = title
        self.keyboard = PuppeteerKeyboard(bridge, url)

    async def title(self):
        self._title = await asyncio.to_thread(
            self.bridge.request, {"op": "title", "url": self.url}
        )
        return self._title

    async def bring_to_front(self):
        return await asyncio.to_thread(
            self.bridge.request, {"op": "bring_to_front", "url": self.url}
        )

    async def evaluate(self, expression):
        return await asyncio.to_thread(
            self.bridge.request, {
                "op": "evaluate", "url": self.url, "expression": expression
            }
        )
    ...
    async def query_selector(self, selector):
        result = await asyncio.to_thread(
            self.bridge.request, {
                "op": "query_one", "url": self.url, "selector": selector
            }
        )
        if result is None:
            return None
        return PuppeteerElement(self.bridge, self.url, selector, result.get("index", 0))

    async def query_selector_all(self, selector):
        count = await asyncio.to_thread(
            self.bridge.request, {
                "op": "query_all_count", "url": self.url, "selector": selector
            }
        )
        return [
            PuppeteerElement(self.bridge, self.url, selector, i)
...
class PuppeteerBrowser:
    ...
    async def pages(self):
        ...
        return [
            PuppeteerPage(self.bridge, row["url"], row.get("title", "(no title)"))
            for row in rows
            if row.get("url")
        ]
...
class PuppeteerBridge:
    def __init__(self):
        ...
        if not node:
            raise RuntimeError("Node.js was not found on PATH; QS 4.0 requires Node.js.")
        ...
        try:
            ...
            for page in pages:
                ...
                if input_sel and output_sel:
                    node_name = name
                    if node_name in self.nodes and self.nodes[node_name].url != url:
                        suffix = 2
