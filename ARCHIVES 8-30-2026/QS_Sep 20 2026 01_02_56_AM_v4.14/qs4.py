...
class QSUI:
    ...
    def _send_order_release(self, event):
        ...
        if target is not None and 0 <= source < len(self.send_order) and 0 <= target < len(self.send_order):
            if source != target and self.controller.relay_state == RelayState.IDLE:
...
class QSController:
    ...
    def _response_candidates(self, messages, node):
        ...
        includes both user and assistant bubbles. The exact text QS just
        submitted is therefore not evidence that the model has responded.
        """
        ...
        submitted = (node.last_submitted_text or "").strip()
        if submitted:
            filtered = [
                candidate for candidate in candidates
                if str(candidate).strip() != submitted
            ]
            if len(filtered) != len(candidates):
                self.ui.log(
                    f"[WAIT-DIAG] {node.name}: filtered submitted user message from response candidates.",
                    is_system=True
                )
            candidates = filtered
    ...
    async def _wait_for_response(self, page, node, timeout=45, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            if not send_ready:
                ...
                if poll <= 3:
                    self.ui.log(f"[WAIT-DIAG] {node.name}: send control reports idle/available.", is_system=True)
