...
#   - Allows first-run bootstrap when no QS 2.3 archive exists remotely; the local
...
IMPORTANT LIBRARY RULE: ALL JSON IN THE QUACKSINK LIBRARY AND INDEX MUST BE READ/INGESTED AS JSON OBJECTS.
Do NOT treat a .JSON item as a plain-text file. Parse and use its JSON structure.
This applies to MASTER_INDEX.JSON, WHO_HE_ME.JSON, CARD JSON, and any other JSON encountered in the Library.

QUACKSINK ALSO USES A WHOHEDECIMAL (WHDC) ADDRESS SYSTEM.
WhoHeDecimal provides stable symbolic addresses for concepts, Minds, systems, mechanisms, failures, ideas, history, tools, and processes in the Library.
These addresses are references, not summaries. The authoritative meaning of each address is defined by WHO_HE_ME.JSON.
WhoHeDecimal addresses may be hierarchical, so an address such as 114.357 is a valid symbolic address; do not treat it as an arithmetic decimal number.

At session close, participating Minds may provide their WhoHe addresses in this exact form:
WHOHE_BEGIN
130,140,210
WHOHE_END

The session's WhoHe addresses belong in its CARD/finding aid so the Master Index can expose the conversation's context without requiring the Librarian to read the entire CONVO merely to discover where it belongs. The CONVO remains the faithful historical evidence; the CARD is the finding aid; WHO_HE_ME.JSON is the address dictionary; MASTER_INDEX.JSON is the master directory.
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        # Archive actions stay on one horizontal control row, matching the
        # established QS UI layout. The archive selector remains above them.
