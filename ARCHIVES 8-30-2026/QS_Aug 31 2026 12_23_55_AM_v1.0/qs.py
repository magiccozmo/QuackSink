...
#
# QS 1.0:
#   - Renamed runtime to qs.py.
#   - New archive root: <startup directory>\ARCHIVES
#   - New archive naming: QS_<old timestamp>_v<version>
#   - Startup archive verification against QuackSink GitHub repository.
#   - Exact source comparison for matching-version archives.
#   - Reports newer local/repository versions without silently replacing source.
#   - First-run behavior: no matching archive is an error condition only in the
#     verification report; QS continues so the first archive can be created.
#   - Preserves the MMRC Luma 1.8 relay/DOM behavior.
...
def _repo_raw_url(archive_name):
    return ARCHIVE_RAW_BASE_URL + archive_name + "/" + RUNTIME_FILE


def _fetch_url_text(url, timeout=15):
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "QuackSink-QS-Startup-Checker"}
    )
...
def _github_archive_listing():
    ...
    Returns a list of directory names. A failure here does not prevent QS
    from starting; startup verification simply reports the inability to check.
    """
    api_url = (
    ...
    if not isinstance(data, list):
        raise RuntimeError("GitHub archive listing was not a directory listing.")
...
def verify_repository_version():
    ...
    Returns a dictionary suitable for logging/display. Network failures are
    reported rather than treated as proof of a source mismatch.
    """
    result = {
        "status": "UNKNOWN",
        "message": "",
    ...
    try:
        ...
        result["message"] = f"Could not inspect QuackSink archive: {exc}"
        return result
    ...
    if not versioned:
        ...
        result["message"] = "No archive found in the QuackSink repository."
        return result
    ...
    if not matching:
        if _version_tuple(latest_version) > _version_tuple(VERSION):
            ...
            result["message"] = (
                f"Repository contains newer QS version {latest_version}. "
    ...
    # If more than one directory carries this version, check each until one
    # matches exactly. This makes duplicate timestamp archives harmless.
    ...
    # A matching version exists, but none of its archived source files exactly
    # matches the running source.
    result["status"] = "MISMATCH"
    result["message"] = (
        f"ERROR: QS {VERSION} archive exists, but archived qs.py does NOT "
        f"match the running qs.py."
    )
    return result
...
def startup_version_check():
    print("=" * 64)
    print(f"{PROGRAM_NAME} QS {VERSION} STARTUP VERSION CHECK")
    print(f"Runtime: {os.path.abspath(__file__)}")
    print(f"Archive repository: {ARCHIVE_REPO_TREE_URL}")
    print("-" * 64)
    ...
    if result["status"] == "REPO_NEWER":
        print(f"NEWEST ARCHIVE: QS {result['latest_version']}")
        print(f"ARCHIVE LINK: {result['latest_url']}")
    elif result["status"] in ("NO_ARCHIVE", "NO_MATCHING_ARCHIVE"):
        if result.get("latest_version"):
            print(f"NEWEST ARCHIVE FOUND: QS {result['latest_version']}")
            print(f"ARCHIVE LINK: {result['latest_url']}")

    if result["status"] == "MISMATCH":
        print("STARTUP HALTED: version-matched source integrity failure.")
        return False, result
...
logger.info("=============================================================")
logger.info("QuackSink QS %s STARTING", VERSION)
...
logger.info("Startup version check: %s", VERSION_CHECK["status"])
logger.info("%s", VERSION_CHECK["message"])
logger.info("Source archived in active archive.")
logger.info("Configuration archived in active archive when present.")
logger.info("=============================================================")
...
def save_config(config):
    try:
        ...
        archive_config = os.path.join(ACTIVE_ARCHIVE_DIR, CONFIG_FILE)
