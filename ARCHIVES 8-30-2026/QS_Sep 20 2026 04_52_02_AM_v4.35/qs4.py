...
VOICE_CAPABLE_NODES = {"GPT", "Gemini", "DeepSeek"}
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        tk.Label(
            order_title,
            text="drag nodes left/right to choose fan-out order",
    ...
    def _render_nodes(self):
        ...
        for name in keys:
            ...
            node_frame = tk.Frame(row, bg="#121212")
            node_frame.pack(side=tk.LEFT, padx=8)
            ...
            node._voice_var = tk.BooleanVar(value=node.voice_enabled)
            voice_supported = node.name in VOICE_CAPABLE_NODES
            if voice_supported:
                ...
                node._voice_btn = tk.Button(
                    ...
                    font=("Arial", 9, "bold"), height=1, padx=5
                )
            else:
                node._voice_btn = tk.Button(
                    node_frame,
            ...
            node._voice_btn.pack(side=tk.LEFT, padx=(4, 0))
        self._render_send_order()
    ...
    def _toggle_node_voice(self, name):
        ...
        if not node:
            return
        if name not in VOICE_CAPABLE_NODES:
            node.voice_enabled = False
            self.log(f"[VOICE] {name} voice is not available in QS 4.31.", is_system=True)
            return

        node.voice_enabled = not bool(node.voice_enabled)
        if node._voice_var is not None:
            node._voice_var.set(node.voice_enabled)
        ...
        if node.voice_enabled:
            self.log(f"[VOICE] {name} voice enabled.", is_system=True)
        else:
            self.log(f"[VOICE] {name} voice disabled.", is_system=True)

    def is_node_voice_enabled(self, node):
        return bool(node and node.voice_enabled and node.name in VOICE_CAPABLE_NODES)
...
class QSController:
            ...
            menu_item = await page.evaluate(r"""
                (() => {
                    ...
                    const exact = (el) => {
                        ...
                        return /^read\s+aloud$/i.test(text) || /^read\s+aloud$/i.test(aria) || /^read\s+aloud$/i.test(title);
                    };
                    const selectors = [
    ...
    async def _click_gemini_listen(self, page):
        """Gemini: latest assistant response -> circular ellipsis button -> Listen."""
        try:
            opened = await page.evaluate(r"""
                (() => {
                    ...
                    const clean = (value) => String(value || '').replace(/\s+/g, ' ').trim();
                    const attrs = (el) => [
                        'aria-label', 'title', 'data-testid', 'data-tooltip',
                        'mattooltip', 'mat-tooltip', 'data-tooltip-text', 'data-tooltip-content'
                    ].map((name) => el.getAttribute(name) || '');
                    const labels = (el) => attrs(el).concat(el.textContent || '').map(clean).filter(Boolean);
                    ...
                    const isEllipsisLabel = (value) =>
                        /^(?:more|more\s+actions?|more\s+options?|more\s+vertical|more[_ -]?vert(?:ical)?|more[_ -]?horiz(?:ontal)?|ellipsis(?:[_ -](?:horizontal|vertical))?)$/i.test(value) ||
                        /^(?:\.\.\.|…|⋯)$/u.test(value);

                    const isEllipsisIcon = (el) => {
                        const iconText = Array.from(el.querySelectorAll('mat-icon,svg,[data-icon],[data-lucide],[class]'))
                            .flatMap((icon) => [
                                icon.textContent || '',
                        ...
                        return /(?:more[_ -]?vert|more[_ -]?horiz|ellipsis(?:[_ -](?:horizontal|vertical))?)/i.test(iconText);
                    };

                    const isMoreButton = (el) => {
                        if (!visible(el)) return false;
                        if (labels(el).some(isEllipsisLabel)) return true;
                        if (isEllipsisIcon(el)) return true;
                        // Angular Material / custom tooltip implementations often put the
                        // tooltip text on an attribute that is not part of our normal set.
                        const tooltipAttrs = Array.from(el.attributes || []).map((a) => clean(a.value));
                        if (tooltipAttrs.some(isEllipsisLabel)) return true;
                        // Tooltip can be carried by a descendant rather than the button.
                        return Array.from(el.querySelectorAll('*')).some((node) =>
                            [
                                'aria-label', 'title', 'data-tooltip', 'mattooltip',
                                'mat-tooltip', 'data-tooltip-text', 'data-tooltip-content'
                            ].some((name) => isEllipsisLabel(clean(node.getAttribute(name))))
                        );
                    };

                    const buttonSelector = 'button,[role="button"],[mattooltip],[data-tooltip],[aria-label]';
                    let target = null;
                    let targetReason = '';

                    // First choice: an actual button/control under the response or its
                    // immediate action-strip ancestors.
                    let scope = latest;
                    for (let depth = 0; depth <= 12 && scope && !target; depth += 1) {
                        const controls = Array.from(scope.querySelectorAll(buttonSelector)).filter(visible);
                        const matches = controls.filter(isMoreButton);
                        if (matches.length) {
                            target = matches[matches.length - 1];
                            targetReason = `explicit-ellipsis depth=${depth}`;
                            break;
                        }
                        scope = scope.parentElement;
                    }

                    // Second choice: use tooltip attributes literally. This is aimed at the
                    // circular Gemini button whose visible content is an icon, not text.
                    if (!target) {
                        const tooltipNamed = Array.from(document.querySelectorAll(buttonSelector))
                            .filter(visible)
                            .filter((el) => Array.from(el.attributes || []).some((a) =>
                                /tooltip/i.test(a.name) && /^(?:more|\.\.\.|…|⋯)$/iu.test(clean(a.value))
                            ));
                        if (tooltipNamed.length) {
                            const rr = latest.getBoundingClientRect();
                            target = tooltipNamed.sort((a,b) => {
                                const ar = a.getBoundingClientRect(), br = b.getBoundingClientRect();
                                return Math.abs((ar.top + ar.height/2) - rr.bottom) - Math.abs((br.top + br.height/2) - rr.bottom);
                            })[0];
                            targetReason = 'tooltip-attribute proximity';
                        }
                    }

                    // Final deliberate fallback: Gemini's control is a small circular icon
                    // button. Restrict candidates to the action area nearest the latest
                    // response and prefer square ~24-48px buttons with little/no text and
                    // an SVG/material icon inside. This never searches another browser page.
                    if (!target) {
                        const rr = latest.getBoundingClientRect();
                        const candidates = Array.from(document.querySelectorAll('button,[role="button"]'))
                            ...
                            .map((el) => {
                                const r = el.getBoundingClientRect();
                                const text = clean(el.textContent || '');
                                const hasIcon = !!el.querySelector('svg,mat-icon,[data-icon],[data-lucide]');
                                const square = r.width >= 20 && r.width <= 56 && r.height >= 20 && r.height <= 56 && Math.abs(r.width-r.height) <= 12;
                                const near = Math.abs((r.top + r.height/2) - rr.bottom);
                                const below = (r.top + r.height/2) >= rr.bottom;
                                let score = 0;
                                if (square) score += 5;
                                if (hasIcon) score += 3;
                                if (!text || text.length <= 3) score += 4;
                                if (below) score += 2;
                                score -= Math.min(near / 120, 8);
                                // Penalize obvious non-menu actions when labels expose them.
                                if (/^(?:like|dislike|share|edit|copy|retry)$/i.test(text)) score -= 6;
                                return {el, score, text, w:r.width, h:r.height, near};
                            })
                            .filter((item) => item.score > 2)
                            .sort((a,b) => b.score - a.score);
                        if (candidates.length) {
                            target = candidates[0].el;
                            targetReason = `circular-icon fallback score=${candidates[0].score.toFixed(1)}`;
                        }
                    }
                    ...
                    if (!target) {
                        ...
                        return {ok:false, reason:'Gemini circular ellipsis button not found.', diagnostics};
                    }
        ...
        if not opened or not opened.get('ok'):
            ...
            return False
        self.ui.log(f"[VOICE] Gemini ellipsis button clicked ({opened.get('reason', 'target')}).", is_system=True)
        ...
        try:
            listened = await page.evaluate(r"""
                (() => {
                    ...
                    const controls = Array.from(document.querySelectorAll('[role="menuitem"],button,[role="button"],[mat-menu-item]'))
                        .filter(exactListen);
                    if (controls.length) {
                        controls[controls.length - 1].click();
                        return {ok:true};
                    }
                    return {ok:false, reason:'Gemini Listen menu item not found.'};
                })()
            """)
        except Exception as exc:
        ...
        if not listened or not listened.get('ok'):
            ...
            return False
        self.ui.log("[VOICE] Gemini Listen clicked.", is_system=True)
        return True
    ...
    async def _click_deepseek_read_aloud(self, page, node):
        """DeepSeek: latest response -> inline Read aloud control."""
        try:
            clicked = await page.evaluate(r"""
                (() => {
                    ...
                    const candidates = Array.from(latest.querySelectorAll('button,[role="button"],[data-testid]')).filter(exactRead);
                    if (!candidates.length) return {ok:false, reason:'DeepSeek inline Read aloud control not found on latest response.'};
        ...
        if not clicked or not clicked.get('ok'):
            ...
            return False
        self.ui.log("[VOICE] DeepSeek Read aloud clicked.", is_system=True)
        return True
        ...
        try:
            return bool(await page.evaluate(r"""
                (() => Array.from(document.querySelectorAll('button,[role="button"],[data-testid]')).some((el) => {
                    ...
                    if (style.display === 'none' || style.visibility === 'hidden' || rect.width <= 0 || rect.height <= 0) return false;
                    const label = [el.getAttribute('aria-label') || '', el.getAttribute('title') || '', el.getAttribute('data-testid') || '', el.textContent || ''].join(' ').replace(/\s+/g, ' ').trim();
                    return /\b(?:pause|stop)(?:\s+(?:reading|reading\s+aloud))?\b/i.test(label);
                }))()
            """))
        except Exception:
            return False
    ...
    async def _maybe_speak_node(self, page, node):
        """Speak a completed response when that node's voice toggle is enabled."""
        if not self.ui.is_node_voice_enabled(node):
            return

        # Voice is a browser-page action.  Verify the page object is still the
        # exact tab recorded for this node before touching any DOM in it.
        ...
        if node.name == 'GPT':
            ...
            self.ui.log(f"[VOICE] {node.name} voice path is unavailable in QS 4.31.", is_system=True)
            return
            ...
            for node in self.relay.active_nodes:
                ...
                if responded:
                    ...
                    if clean:
                        ...
                        self.ui.log(f"\n{relay_msg}\n")
                        await self._maybe_speak_node(page, node)
