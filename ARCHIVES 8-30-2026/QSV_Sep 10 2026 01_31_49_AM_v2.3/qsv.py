...
class QSController:
            ...
            async with async_playwright() as p:
                ...
                for node in self.relay.active_nodes:
                    ...
                    if responded:
                        ...
                        if clean and not dropped:
                            await self._read_aloud_latest_response(page, node)
                        if dropped:
    ...
    async def _read_aloud_latest_response(self, page, node):
        """
        For GPT only, click the Read aloud control attached to the newest
        assistant response.  This is intentionally best-effort: failure to
        start audio must never turn a successful relay response into a relay
        failure.
        """
        if node.name != "GPT":
            return False
        ...
        try:
            # First identify the newest visible assistant response and return
            # a unique CSS path to a matching Read aloud control inside it.
            selector = await page.evaluate("""
                (responseSelector) => {
                    ...
                    const responseSelector = arguments[0];
                    let responses = [];
                    try {
                    ...
                    const controls = [...response.querySelectorAll('button, [role="button"]')];
                    const wanted = controls.find(el => {
                        ...
                        const haystack = [aria, title, testid, text].join(' ');
                    ...
                    return wanted ? uniqueSelector(wanted) : null;
                }
            """, node.output_selector)

            if not selector:
                self.ui.log(
                    "[READ ALOUD] GPT: Read aloud control not found on newest response.",
                    is_system=True
                )
                return False
            ...
            if not button or not await button.is_visible():
                self.ui.log(
                    "[READ ALOUD] GPT: located control is not visible.",
                    is_system=True
                )
                return False
            ...
            await button.click()
            self.ui.log("[READ ALOUD] GPT: clicked Read aloud on newest response.", is_system=True)
            return True
        except Exception as exc:
            self.ui.log(
