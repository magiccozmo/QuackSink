...
RUNTIME_FILE = "qs6_fixed_v6_10.py"
...
class QSController:
        ...
        try:
            ...
            result = await page.evaluate(rf"""
                (() => {{
                    ...
                    // FAIL CLOSED: never report the previous response as the state of the current one.
                    if (!matches.length) {{
                        return {{state:'unknown', reason:'Exact captured GPT response is not visible', responseMatched:false}};
                    }}

                    const latest = matches[matches.length - 1];
                    const turn = latest.closest('[data-testid^="conversation-turn-"]');
                    if (!turn) {{
                        return {{state:'unknown', reason:'Exact GPT response turn wrapper is not visible yet', responseMatched:true}};
                    }}
                    ...
                    const active = controls.find((el) => labels(el).some((value) =>
                        /\b(?:pause|stop|cancel)\b/i.test(value) &&
                        /(?:reading|read aloud|speech|speaking|playback|voice)?/i.test(value)
                    ));
                    if (active) return {{
                        state:'active',
                    ...
                    // Some Chromium speech implementations expose playback as page media
                    // rather than changing the response action label. This is still safe here
                    // because the page itself is the exact GPT page on which QS clicked Read aloud.
                    try {{
                        if (Array.from(document.querySelectorAll('audio,video')).some((el) =>
                            !el.paused && el.readyState >= 2)) {{
                            return {{state:'active',label:'browser media playback',responseMatched:true}};
                        }}
                    }} catch (_) {{}}
                    try {{
                        if (window.speechSynthesis && window.speechSynthesis.speaking) {{
                            return {{state:'active',label:'window.speechSynthesis',responseMatched:true}};
                        }}
                    }} catch (_) {{}}

                    return {{state:'unknown', reason:'No exact-turn GPT playback state exposed', responseMatched:true}};
                }})()
            """)
    ...
    async def _wait_for_voice_playback(self, page, node, response_text=None):
        ...
        # ------------------------------------------------------------------
        # GPT: exact-turn + browser-activity completion path.
        # ------------------------------------------------------------------
        if node.name == 'GPT':
            words = len(re.findall(r"\S+", response_text or ""))

            # 150 wpm approximation with a small completion grace. The old 120 wpm
            # estimate plus 8-second grace was producing conspicuous post-speech lag.
            estimated_speech = max(5.0, min(180.0, words / 2.5))
            fallback_after = max(12.0, estimated_speech + 2.0)
            startup_deadline = min(12.0, fallback_after)
            ...
            while elapsed() < startup_deadline:
                ...
                if state.get('state') == 'active':
                    saw_playback = True
                    self.ui.log(
                        f"[VOICE] GPT playback started; state={state.get('label', 'active')!r}; "
                        "waiting for native playback to finish.",
                        is_system=True
                    )
                    break

                if await self._native_voice_activity_visible(page):
                    saw_playback = True
                    self.ui.log(
                        "[VOICE] GPT browser media/speech playback became observable; "
                        "waiting for activity to end.",
                        is_system=True
                    )
                    break
            ...
            if not saw_playback:
                self.ui.log(
                    f"[VOICE] GPT native playback state remains UNKNOWN after {elapsed():.1f}s; "
                    f"tighter bounded release window is {fallback_after:.1f}s.",
                    is_system=True
                )
                ...
                while elapsed() < fallback_after:
                    ...
                    if await self._native_voice_activity_visible(page):
                        saw_playback = True
                        self.ui.log(
                            "[VOICE] GPT browser media/speech playback became observable late.",
                            is_system=True
                        )
                        break
                    await asyncio.sleep(0.20)
            ...
            if not saw_playback:
                self.ui.log(
                    ...
                    f"releasing relay after tighter bounded UNKNOWN window ({elapsed():.1f}s).",
                    is_system=True
                )
                return False
            ...
            while elapsed() < 300.0:
                ...
                if current == 'active':
                    ...
                    if idle_streak >= 2:
                        self.ui.log(
                            "[VOICE] GPT playback complete; exact current response voice control is idle.",
                            is_system=True
                        )
                        return True
                else:
                    idle_streak = 0
                    media_active = await self._native_voice_activity_visible(page)
                    if media_active:
                        browser_idle_streak = 0
                        last_active = elapsed()
                    else:
                        browser_idle_streak += 1
                        # Once actual browser media/speech was observed, three consecutive
                        # inactive samples are enough to end the voice wait even if GPT's
                        # exact action label disappears instead of returning to Read aloud.
                        if browser_idle_streak >= 3 and elapsed() - last_active >= 0.6:
                            self.ui.log(
                                "[VOICE] GPT playback complete; browser media/speech activity ended "
                                "while exact-turn control telemetry was unavailable.",
                                is_system=True
                            )
                            return True
                    ...
                    if elapsed() >= fallback_after and elapsed() - last_active >= 8.0:
                        self.ui.log(
                            "[VOICE] GPT playback state became UNKNOWN after playback; "
                            f"releasing after bounded safety window ({elapsed():.1f}s).",
                            is_system=True
                        )
                        return False
