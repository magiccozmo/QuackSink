...
logger.info("=============================================================")
logger.info("MMRC Luma 1.4 STARTING")
...
class NodeState:
    ...
    def can_toggle(state):
        return state in (NodeState.OFF, NodeState.FAILED)
...
class DOMAnalyzer:
    ...
    async def analyze(page: Page):
        result = {"input": None, "assistant": None, "user": None}

        input_selectors = await page.evaluate("""
            () => {
                const candidates = [];
                const textarea = document.querySelector('textarea');
                if (textarea && textarea.offsetHeight > 0) candidates.push('textarea');
                const contenteditable = document.querySelector('[contenteditable="true"]');
                if (contenteditable && contenteditable.offsetHeight > 0)
                    candidates.push('[contenteditable="true"]');
                const role_textbox = document.querySelector('div[role="textbox"]');
                if (role_textbox && role_textbox.offsetHeight > 0)
                    candidates.push('div[role="textbox"]');
                if (candidates.length === 0) {
                    const fallback = document.querySelector('input[type="text"], input:not([type])');
                    if (fallback) candidates.push(fallback.tagName.toLowerCase() + '[type="text"]');
                }
                return candidates;
            }
        """)
        ...
        assistant_selectors = await page.evaluate("""
            () => {
                const candidates = [];
                const by_role = document.querySelector('[data-role="assistant"]');
                if (by_role) candidates.push('[data-role="assistant"]');
                const model_response = document.querySelector('model-response');
                if (model_response) candidates.push('model-response');
                const divs = document.querySelectorAll(
                    'div[class*="assistant"], div[class*="message"], div[class*="response"]'
                );
                for (const d of divs) {
                    if (d.innerText && d.innerText.length > 20) {
                        const classes = d.className.split(' ').filter(c => c).join('.');
                        candidates.push(d.tagName.toLowerCase() + '.' + classes);
                        break;
                    }
                }
                if (candidates.length === 0) {
        ...
        user_selectors = await page.evaluate("""
            () => {
                const candidates = [];
                const by_role = document.querySelector('[data-role="user"]');
                if (by_role) candidates.push('[data-role="user"]');
                const divs = document.querySelectorAll('div[class*="user"], div[class*="human"]');
                for (const d of divs) {
                    if (d.innerText && d.innerText.length > 10) {
                        const classes = d.className.split(' ').filter(c => c).join('.');
                        candidates.push(d.tagName.toLowerCase() + '.' + classes);
                        break;
                    }
...
class MMRCUI:
    def __init__(self, controller):
        self.root = tk.Tk()
        self.root.title("MMRC Luma 1.4 - Structural DOMAnalyzer")
    ...
    def _build_ui(self):
        tk.Label(
            self.root, text="MULTI-MIND RELAY CORE - Luma 1.4",
        ...
        self.archive_box = ttk.Combobox(
            ...
            state="readonly", width=38, font=("Arial", 12, "bold")
        )
    ...
    def _render_nodes(self):
        ...
        for idx, name in enumerate(keys):
            ...
            var.trace("w", lambda *args, n=name: self._on_toggle(n))
            node._var = var
            cb = tk.Checkbutton(
                row,
                text=f"{state_icon} {node.icon} {node.name}{indicator}",
                variable=var, font=("Arial", 14, "bold"), bg="#121212",
    ...
    def _archive_display_name(path):
        # The directory name itself is deliberately used as the display
        # value. It is already monkey-readable and is also the canonical
        # archive identifier on disk.
...
class MMRCController:
    ...
    async def _wait_for_response(self, page, node, timeout=45):
        ...
        while asyncio.get_event_loop().time() - start < timeout:
            ...
            if ready_streak >= 3:
                self.ui.log(
                    f"[WAIT] {node.name}: {'response finalized' if saw_busy else 'send ready'}.",
                    is_system=True
                )
                return True
    ...
    async def _get_response(self, page, node):
        ...
        try:
            messages = await page.evaluate(f"""
                () => {{
                    const elements = document.querySelectorAll({json.dumps(output_selector)});
                    const results = [];
                    for (const el of elements) {{
            ...
            for last_message in reversed(messages):
                matches = list(re.finditer(r'🟩.*?🔚', last_message, re.DOTALL))
                if matches:
                    frame_content = matches[-1].group()[1:-1].strip()
                    if (
                        frame_content and
                        "MMRC" not in frame_content and
                        "Multi-Mind Relay Core" not in frame_content
                    ):
                        return frame_content
            return messages[-1].strip()
        except Exception as exc:
            self.ui.log(f"[RESPONSE] Error: {exc}", is_system=True)
        return "No response text extracted from DOM."
...
if __name__ == "__main__":
    logger.info("Launching MMRC Luma 1.4.")
