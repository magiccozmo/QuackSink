            ...
            (responseSelector) => {
                ...
                function wanted(el) {
                    ...
                    return haystack.includes('read aloud') ||
                           ...
                           haystack.includes('listen to response') ||
                           haystack.includes('listen to this response') ||
                           haystack.includes('play audio');
                }
                function uniqueSelector(el) {
                    if (el.id) {
                        const id = '#' + CSS.escape(el.id);
                ...
                if (newest) {
                    const scoped = [...newest.querySelectorAll('button, [role="button"]')].find(wanted);
                    if (scoped) return uniqueSelector(scoped);
                }
                const globalButtons = [...document.querySelectorAll('button, [role="button"]')].filter(wanted);
                if (globalButtons.length) return uniqueSelector(globalButtons[globalButtons.length - 1]);
                return null;
            }
        """, node.output_selector)
