...
class QSUI:
    ...
    def _render_nodes(self):
        ...
        # QS 5.1 UI: node controls are arranged in a wrapped grid of cards rather
        # than one long horizontal strip.  The voice selector lives on its own
        # second line so the node roster cannot disappear off the right edge.
        # Three cards per row is the normal 1200px-wide layout; narrower windows
        # automatically use fewer columns.
        try:
            available_width = max(1, self.root.winfo_width() - 40)
        except Exception:
            available_width = 1200
        card_min_width = 360
        columns = max(1, min(3, available_width // card_min_width))
        ...
        for index, name in enumerate(keys):
            ...
            node_frame = tk.Frame(row, bg="#1a1a1a", padx=7, pady=5, highlightthickness=1, highlightbackground="#333333")
            ...
            node._voice_btn.pack(side=tk.RIGHT, padx=(6, 0))

            selector_row = tk.Frame(node_frame, bg="#1a1a1a")
            selector_row.pack(fill=tk.X, pady=(5, 0))

            tk.Label(
                selector_row, text="VOICE:",
                font=("Arial", 9, "bold"), bg="#1a1a1a", fg="#00FFCC"
            ).pack(side=tk.LEFT, padx=(0, 6))

            selected_display = VOICE_SELECTION_TO_DISPLAY.get(
                node.voice_selection,
                "NATIVE UI"
            )
            node._voice_combo = ttk.Combobox(
                selector_row,
                values=VOICE_DISPLAY_OPTIONS,
                state="readonly",
                width=18,
                font=("Arial", 9, "bold")
            )
            node._voice_combo.set(selected_display)
            node._voice_combo.bind(
                "<<ComboboxSelected>>",
                lambda _event, n=name: self._voice_selection_changed(n)
            )
            # Keep the closed control compact; expand it while selected so the
            # complete voice ID is easy to inspect without permanently consuming
            # the horizontal space needed by the node roster.
            node._voice_combo.bind(
                "<FocusIn>",
                lambda event: event.widget.configure(width=34)
            )
            node._voice_combo.bind(
                "<FocusOut>",
                lambda event: event.widget.configure(width=18)
            )
            node._voice_combo.pack(side=tk.LEFT, fill=tk.X, expand=True)
    ...
    def _voice_selection_changed(self, name):
        node = self.nodes.get(name)
        if not node or node._voice_combo is None:
            return
        display = node._voice_combo.get()
    ...
    def _refresh_voice_controls(self):
        for node in self.nodes.values():
            ...
            if node._voice_combo is not None:
                node._voice_combo.set(
                    VOICE_SELECTION_TO_DISPLAY.get(node.voice_selection, "NATIVE UI")
                )
