...
#
# Luma 1.4 additions:
# - Explicit MMRC_ROOT path boundary.
# - Historical archive discovery.
# - Selected LOG.txt -> clipboard export.
# - Selected archived SOURCE -> clipboard export.
# - Current running SOURCE -> clipboard export.
# - Export headers identify exact source file.
#
# IMPORTANT PATH RULE:
# All MMRC archives live directly beneath the directory
# from which MMRC was launched.
#
# The active archive NEVER becomes MMRC_ROOT.
...
# MMRC ROOT
#
# This is the directory from which MMRC was launched.
# Archives are ALWAYS children of this directory.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# ARCHIVE DISCOVERY / EXPORT
# ------------------------------------------------------------

def discover_archives():
    """
    Return MMRC archive directories directly beneath MMRC_ROOT.

    Archives are NOT searched recursively.

    Only directories whose names begin with ARCHIVE_PREFIX
    are considered.
    """
    ...
    try:
        for entry in os.scandir(MMRC_ROOT):
            if not entry.is_dir():
                continue
...
def archive_display_name(archive_path):
    return os.path.basename(
        os.path.normpath(archive_path)
    )


def find_archived_source(archive_path):
    """
    Find the source file archived in an archive.

    The source filename is deliberately NOT hard-coded.

    LOG.txt and mmrc_config.json are excluded.
    """
    ...
    try:
        for entry in os.scandir(archive_path):
            ...
            # The startup archive operation copies __file__,
            # so the source is expected to be a Python file.
            if entry.name.lower().endswith(".py"):
                return entry.path

    except Exception as exc:
        logger.error(
            "Archived source discovery failed for %s: %s",
            archive_path,
            exc
        )
...
def read_exact_text_file(path):
    """
    Read the exact textual contents of a file.

    No parsing or transformation is performed.
    """
...
def build_clipboard_export(
    export_type,
    source_path,
    body
):
    """
    Build the human-mediated export payload.

    The header identifies the exact file used.
    The body is the exact text read from that file.
    """
    ...
    if export_type == "LOG":
        title = "MMRC LOG EXPORT"

    elif export_type == "SOURCE":
        title = "MMRC SOURCE EXPORT"

    else:
        title = "MMRC TEXT EXPORT"
...
class ExportManager:
    """
    Local artifact export only.

    This class has NO node, browser, relay, or network role.

    It reads a selected file and places its text onto the
    system clipboard through the Tk root.
    """
    ...
    def copy_file_to_clipboard(
        ...
        try:
            body = read_exact_text_file(
                file_path
            )

            payload = build_clipboard_export(
                export_type,
                file_path,
                body
            )
            ...
            # Ask Tk to keep the clipboard contents available
            # after MMRC remains running.
            self.root.update()
...
class MMRCUI:
    def __init__(self, controller):
        ...
        self.root.title(
            "MMRC Luma 1.4 - Structural DOMAnalyzer"
        )
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self.root,
            text="MULTI-MIND RELAY CORE - Luma 1.4",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(
        ...
        tk.Button(
            ...
            command=self.refresh_archive_list,
            bg="#555555",
            fg="white",
            font=("Arial", 10, "bold"),
            padx=8
        ...
        tk.Button(
            export_frame,
            text="COPY SELECTED LOG TO CLIPBOARD",
        ...
        tk.Button(
            export_frame,
            text="COPY SELECTED SOURCE TO CLIPBOARD",
            command=self.copy_selected_source,
            bg="#664488",
            fg="white",
            font=("Arial", 10, "bold"),
            padx=8
        ...
        current_source_frame = tk.Frame(
            self.root,
            bg="#121212"
        )

        current_source_frame.pack(
            pady=3
        )

        tk.Button(
            current_source_frame,
            text="COPY CURRENT SOURCE TO CLIPBOARD",
            command=self.copy_current_source,
            bg="#885522",
            fg="white",
            font=("Arial", 10, "bold"),
            padx=8
    ...
    def refresh_archive_list(self):
        self.archive_paths = discover_archives()
        ...
        if display_names:
            ...
            # Prefer the active archive if present.
            active_index = None

            active_normalized = os.path.normcase(
                os.path.normpath(
                    ACTIVE_ARCHIVE_DIR
                )
            )
            ...
            self.archive_box.set("No MMRC archives found")

    def _get_selected_archive(self):
        index = self.archive_box.current()

        if (
            index < 0
            or index >= len(self.archive_paths)
        ):
            return None

        return self.archive_paths[index]

    def copy_selected_log(self):
        archive_path = (
            self._get_selected_archive()
        )

        if not archive_path:
            self.log(
                "[EXPORT] No archive selected.",
                is_system=True
            )
            return
        ...
        success, detail = (
            self.export_manager.copy_file_to_clipboard(
                log_path,
                "LOG"
            )
        )
        ...
        if success:
            self.log(
                "[EXPORT] LOG copied to clipboard: "
                f"{archive_display_name(archive_path)}/LOG.txt",
                is_system=True
            )

        else:
            self.log(
                f"[EXPORT] LOG copy failed: {detail}",
                is_system=True
            )

    def copy_selected_source(self):
        archive_path = (
            self._get_selected_archive()
        )
        ...
        source_path = find_archived_source(
            archive_path
        )

        if not source_path:
            self.log(
                "[EXPORT] No archived source found in "
                f"{archive_display_name(archive_path)}.",
                is_system=True
            )
            return
        ...
        if success:
            self.log(
                "[EXPORT] Archived source copied to clipboard: "
                f"{os.path.relpath(source_path, MMRC_ROOT)}",
                is_system=True
            )

        else:
            self.log(
                f"[EXPORT] Source copy failed: {detail}",
                is_system=True
            )

    def copy_current_source(self):
        ...
        if success:
            self.log(
                "[EXPORT] Current source copied to clipboard: "
                f"{os.path.relpath(source_path, MMRC_ROOT)}",
                is_system=True
            )
            ...
            self.log(
                f"[EXPORT] Current source copy failed: {detail}",
                is_system=True
            )
