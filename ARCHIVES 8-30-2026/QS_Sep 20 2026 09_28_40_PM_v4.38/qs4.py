...
# QS 4.38 DEVELOPMENT:
#   - Adds an Aisha-specific response extractor based on the old DRS/MMRC
#     locked-response-capture behavior recovered from the project archive.
#   - Aisha's UI can re-ingest the full transcript/history wrapper during parsing,
#     so the generic all-message extractor can see old turns as fresh signal.
#   - For Aisha only, QS now extracts exactly one latest message/response node,
#     preferring explicit message identity hooks and otherwise selecting the
#     deepest/latest message-like node rather than reading the transcript wrapper.
#   - The general QS capture path is unchanged. Aisha's quirk is isolated at the
#     browser-extraction boundary so it cannot alter other Minds' response logic.
...
class QSController:
    ...
    def _response_candidates(self, messages, node):
        ...
        # Aisha's adapter intentionally returns at most one latest DOM node.
        # The fallback below therefore compares that node to the pre-submit
        # latest node instead of assuming a growing transcript list.
    ...
    async def _capture_aisha_latest(self, page, node):
        ...
        transcript. QS 4.38 keeps that behavior isolated to Aisha.
        """
        try:
