...
def startup_version_check():
    ...
    if result["status"] in ("NO_ARCHIVE", "NO_MATCHING_ARCHIVE"):
        ...
        print("WARNING: NO AUTHORIZED ARCHIVE FOUND.")
        print("QS will continue. Enter at your own risk.")
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        tk.Button(
            archive_frame, text="WHAT CAN THIS DO?",
            command=self._open_what_can_this_do,
    ...
    async def _do_handshake(self, node):
        try:
            async with async_playwright() as p:
                ...
                self.ui.log(
                    f"[INTRO] {node.name}: embedded Library preamble sent; waiting for response completion.",
                    is_system=True
                )
                preamble_responded = await self._wait_for_response(
                    page, node, timeout=60, is_handshake=False
                )

                if not preamble_responded:
                    node.introduction_failed()
                    self.ui.log(
                        f"[INTRO] {node.name}: preamble response timeout.",
                        is_system=True
                    )
                    return

                # The preamble is an introduction phase. Its response is not a
                # protocol announcement; it is deliberately consumed here so
                # the next DOM baseline belongs to the handshake.
