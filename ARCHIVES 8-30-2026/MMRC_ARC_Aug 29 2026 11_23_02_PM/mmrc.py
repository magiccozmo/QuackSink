...
#   - Response-wait/stabilization logic retained from 2.6.
#   - Exposes exact response text at extraction/parsing boundary.
#   - Separately reports MMRC token presence.
#   - Separately reports complete-frame detection.
#   - Corrects misleading [HEARD_RAW] logging.
#   - No intentional changes to relay behavior.
#
# Core principle:
#
...
class MMRCUI:
    ...
    def __init__(
        ...
        self.root.title(
            "MMRC Luma 2.7 - Diagnostic Response Boundary"
        )
    ...
    def _build_ui(
        ...
        tk.Label(
            self.root,
            text="MULTI-MIND RELAY CORE - Luma 2.7",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(
...
class MMRCController:
    ...
    def _log_response_diagnostics(
        ...
        """
        Diagnostic-only visibility into the response boundary.

        This function does not alter response behavior.
        """
        ...
        logger.info(
            (
                f"[RESPONSE_DIAGNOSTIC] "
                f"{node.name} | "
                f"HAS_START_TOKEN={has_start} | "
                f"HAS_END_TOKEN={has_end} | "
                f"HAS_DROP_TOKEN={has_drop} | "
                f"HAS_COMPLETE_FRAME={has_frame} | "
