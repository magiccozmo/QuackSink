...
class QSController:
    ...
    async def _wait_for_idle_after_introduction(self, page, node, timeout=60):
        start = asyncio.get_event_loop().time()
        stable = 0
        while asyncio.get_event_loop().time() - start < timeout:
            await asyncio.sleep(0.5)
            ready = await self._send_button_state(page)
            if ready:
                stable += 1
                if stable >= 3:
                    self.ui.log(
                        f"[INTRO] {node.name}: composer/browser generation is idle.",
                        is_system=True
                    )
                    return True
            else:
    ...
    async def _do_handshake(self, node):
        try:
            ...
            if not await self._wait_for_idle_after_introduction(page, node, timeout=60):
                node.introduction_failed()
                self.ui.log(
