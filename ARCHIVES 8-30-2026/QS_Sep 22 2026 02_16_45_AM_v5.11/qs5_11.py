...
class KokoroVoiceEngine:
    ...
    def _speak_custom(self, text, voice_id):
        ...
        phonemes = _kokoro_get_phonemes(pipeline, str(text).strip())
        if not phonemes:
        ...
        audio_data = audio.detach().float().cpu().numpy()
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        # QS 5.4: widen only the title-box audition selector so all Kokoro voice IDs are visible.
        ...
        self.title_voice_combo = ttk.Combobox(
            ...
            values=[selection for selection in VOICE_SELECTION_OPTIONS if selection != VOICE_NATIVE_UI],
            state="readonly",
            width=28,
            font=("Arial", 10)
        )
...
class QSController:
    ...
    async def _submit(self, page, node, input_element, selector):
        ...
        # QS 5.11: use a real page-targeted CDP Enter before the older
        # synthetic DOM KeyboardEvent fallback. Manual Enter succeeds because
        # the browser receives a native key event; a JS-created KeyboardEvent
        # is untrusted and some chat UIs ignore it.
        ...
        # Last resort: retain the older synthetic DOM event path.
        try:
            self.ui.log(f"[SEND] SUBMIT ATTEMPT (Enter/DOM fallback): {node.name}", is_system=True)
            await asyncio.to_thread(
                self.browser.bridge.request,
            ...
            self.ui.log(f"[SEND] Enter DOM fallback failed for {node.name}: {exc}", is_system=True)
        ...
        while True:
            ...
            if is_handshake:
                ...
                if handshake_frame:
                    ...
                    self.ui.log(
                        f"[WAIT-DIAG] {node.name}: direct handshake frame observed; "
                        f"stability={handshake_stable_streak}",
                        is_system=True
                    )

                    if handshake_stable_streak >= 4:
                        self.ui.log(
                            f"[WAIT] {node.name}: complete NEW handshake response detected "
                            f"by direct latest-assistant stability.",
                            is_system=True
                        )
                        return True
                else:
