...
#
# QS 4.24 DEVELOPMENT:
#   - Keeps QS 4.23 background-page preparation and document visibility/focus shim unchanged.
#   - Fixes normal-response completion gating: a disabled Send button is not treated as proof that
#     generation is still busy. Normal responses now use the explicit browser generation/busy state
#     together with output stability, matching the introduction/handshake state model.
#
# QS 4.5 RELEASE:
    ...
    async def _wait_for_response(self, page, node, timeout=45, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            if saw_busy and saw_new_response and current_response_text:
                # QS 4.24: use the explicit browser generation state as the
                # completion gate. The Send button is NOT a reliable busy/idle
                # signal because an empty composer can legitimately leave Send
                # disabled even after the assistant has completely finished.
                # Output must still be stable after generation returns to idle,
                # preserving the 4.22 protection against frozen partial output.
