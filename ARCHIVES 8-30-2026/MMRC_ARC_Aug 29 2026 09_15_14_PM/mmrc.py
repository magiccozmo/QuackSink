...
#
# Luma 2.1:
#   - Replaces response count detection with DOM-element identity.
#   - Assigns persistent MMRC identities to assistant DOM elements.
#   - Does NOT treat DOM count changes as proof of a new response.
#   - Does NOT treat DOM text changes as proof of a new response.
#   - Requires an assistant DOM element not present in the
#     pre-submit identity baseline.
#   - Preserves dropout handling.
#   - Preserves startup archival/versioning.
#   - Preserves archive selector and clipboard tools.
#
# IMPORTANT:
#   This version is intentionally conservative.
#   Response detection is based on actual assistant DOM-element
#   identity rather than message count or DOM mutation alone.
...
class Node:
    ...
    def __init__(
        ...
        # Snapshot of assistant DOM identities immediately
        # before submission.
...
class MMRCUI:
    ...
    def __init__(self, controller):
        ...
        self.root.title(
            "MMRC Luma 2.1 - Message Identity Response Detection"
        )
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self.root,
            text="MULTI-MIND RELAY CORE - Luma 2.1",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(
    ...
    def _render_nodes(self):
        ...
        for widget in (
            self.nodes_frame.winfo_children()
        ):
...
class MMRCController:
    ...
    async def _send_message_to_page(
        ...
        try:
            ...
            # ----------------------------------------------------------
            # CRITICAL:
            # Capture actual assistant DOM identities BEFORE submit.
            # ----------------------------------------------------------
            ...
            self.ui.log(
                ...
                f"pre-submit assistant identities.",
                is_system=True
            )
    ...
    async def _submit(
        ...
        # query_selector_all() does not understand
        # Playwright's :has-text() pseudo-selector.
        # Keep the browser-evaluated selector list valid.
    ...
    async def _capture_output_snapshot(
        ...
        """
        Capture assistant DOM elements and assign each element
        a persistent identity inside the page.

        The identity is maintained through a JavaScript WeakMap.

        This means that if the site's framework reorders or
        temporarily moves an existing DOM element, its MMRC
        identity remains attached to that element.

        A genuinely newly-created assistant DOM element receives
        a new identity.
        """
        ...
        try:
            ...
            return await page.evaluate(
                ...
                () => {{
                    ...
                    if (
                        !window.__mmrcAssistantIdentityMap
                    ) {{

                        window.__mmrcAssistantIdentityMap =
                            new WeakMap();

                        window.__mmrcAssistantIdentityCounter =
                            0;
                    }}
            ...
            logger.error(
                "Output snapshot failed for %s: %s",
                node.name,
                exc
            )
    ...
    async def _response_frame_state(
        ...
        """
        Determine whether a genuinely NEW assistant DOM element
        exists.

        Luma 2.1 explicitly rejects the following as sufficient
        evidence:

            - assistant count changed
            - assistant text changed
            - DOM order changed
            - send button became ready

        A response is NEW only when an assistant DOM element has
        an MMRC identity that was absent from the pre-submit
        baseline.
    ...
    async def _wait_for_response(
        ...
        """
        Luma 2.1 response waiter.

        Send-button readiness is diagnostic only.

        Response success requires:

            1. A genuinely NEW assistant DOM identity.
            2. That new message eventually contains:
                   🟩 ... 🔚
               OR:
                   🛑

        A DOM count change alone is never accepted.
        """

        self.ui.log(
            f"[WAIT] {node.name}: "
            f"waiting for NEW assistant DOM identity...",
            is_system=True
        )
        ...
        while (
            ...
            try:
                ...
                if (
                    ...
                    self.ui.log(
                        f"[WAIT] {node.name}: "
                        f"assistant identities changed "
                        f"(added={len(added)}, "
                ...
                if new_messages:
                    ...
                    for item in new_messages:
                        ...
                        self.ui.log(
                            f"[WAIT] {node.name}: "
                            f"NEW assistant identity: "
                            f"{item.get('identity')!r} "
                            f"text={item.get('text', '')[:180]!r}",
                            is_system=True
                        )
                ...
                if state["frame"]:
                    ...
                    self.ui.log(
                        f"[WAIT] {node.name}: "
                        f"complete NEW MMRC frame detected.",
                        is_system=True
                    )

                    return True

                # A new DOM identity may appear before generation
                # is complete. Continue polling until the frame
                # delimiter or dropout appears.
        ...
        try:
            ...
            if new_ids:
                ...
                self.ui.log(
                    f"[DIAG] {node.name}: "
                    f"new identities={sorted(new_ids)}",
                    is_system=True
                )
            ...
            if messages:
                ...
                self.ui.log(
                    f"[DIAG] {node.name}: "
                    f"newest assistant identity="
                    f"{newest.get('identity')!r} "
                    f"text={preview!r}",
                    is_system=True
                )
            ...
            self.ui.log(
                f"[DIAG] {node.name}: "
                f"diagnostic collection failed: {exc}",
                is_system=True
            )
    ...
    async def _get_response(
        ...
        # IMPORTANT:
        # Never fall back to messages[-1] here.
        #
        # If we cannot establish that the message is genuinely
        # new, returning the newest old message recreates the
        # exact false-positive bug this version is designed to fix.

        if not candidates:

            return (
                "No newly identified assistant "
                "response found."
            )

        # First preference:
        # newest complete MMRC frame.
        ...
        # Diagnostic fallback:
        # The identity is known to be new, but the MMRC frame
        # was not found.
    ...
    def _parse_response(
        ...
        # Dropout is independently recognized.
