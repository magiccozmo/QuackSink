...
logger.info("=============================================================")
logger.info("MMRC Luma 1.6 STARTING")
        ...
        result["assistant"] = await page.evaluate(
            ...
            () => {
                ...
                function uniqueSelector(el) {
                    ...
                    const attrs = [
                        ['data-message-author-role', el.getAttribute('data-message-author-role')],
                ...
                for (const selector of selectors) {
                    ...
                    for (const el of els) {
                        if (visible(el) && (el.innerText || '').trim().length > 20) {
                            found.push(el);
                        }
                    }
                }

                // Prefer the most recent visible candidate with meaningful text.
                if (found.length) return uniqueSelector(found[found.length - 1]);
                return null;
            }
            """
        )
...
class MMRCUI:
    def __init__(self, controller):
        self.root = tk.Tk()
        self.root.title("MMRC Luma 1.6 - Structural DOMAnalyzer")
...
class MMRCController:
    ...
    async def _response_frame_state(self, page, node):
        """
        Inspect the discovered output area for a complete MMRC frame.

        This is a completion signal independent of the composer button.
        Some clients do not expose a reliable Send/Stop transition, so
        a complete 🟩 ... 🔚 frame is sufficient evidence that the node has
        produced an MMRC response.
        """
    ...
    async def _wait_for_response(self, page, node, timeout=45):
        """
        Wait for a completed node response.

        Primary signal: the node's UI returns to a usable Send state after
        generation has been observed as busy.

        Independent fallback: a complete MMRC 🟩 ... 🔚 frame appearing in
        the discovered output DOM. This matters because some web clients
        do not expose a stable Send/Stop transition.
        """
        self.ui.log(
        ...
        while asyncio.get_event_loop().time() - start < timeout:
            ...
            if frame_ready:
                ...
                if frame_streak >= 2:
                    self.ui.log(
                        f"[WAIT] {node.name}: complete MMRC frame detected.",
                        is_system=True
                    )
                    return True
            else:
...
if __name__ == "__main__":
    logger.info("Launching MMRC Luma 1.6.")
