...
class QSController:
    ...
    async def _find_native_read_aloud_action(self, page, node, open_menu=False):
        ...
        # Look for the response's More actions control. If the action toolbar is rendered
        # outside the response element, fall back to visible buttons that mention More.
        candidates = []
        try:
            candidates.extend(await response.query_selector_all('button, [role="button"]'))
        except Exception:
            pass
        try:
            parent = response.locator('..')
            candidates.extend(await parent.query_selector_all('button, [role="button"]'))
        except Exception:
            pass
        try:
            candidates.extend(await page.query_selector_all('button, [role="button"]'))
        except Exception:
            pass
        ...
        if open_menu:
            try:
                ...
                self.ui.log(
                    f"[READ ALOUD] GPT: More actions click failed: {exc}",
                    is_system=True
                )
                return None, None
