...
class QSController:
    ...
    async def _wait_for_response(self, page, node, timeout=45, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            try:
                send_ready = bool(await self._send_button_state(page))
            except Exception as exc:
                self.ui.log(
                    f"[WAIT-DIAG] {node.name}: send-state probe failed on poll {poll}: {exc}",
                    is_system=True
                )
                send_ready = False
            ...
            if not send_ready:
                if not saw_busy:
                    self.ui.log(f"[WAIT] {node.name}: node is busy (send control unavailable).", is_system=True)
                saw_busy = True
                idle_streak = 0
            else:
                idle_streak += 1
                if poll <= 3 or idle_streak <= 3:
                    self.ui.log(
                        f"[WAIT-DIAG] {node.name}: send control reports idle/available; idle_streak={idle_streak}",
                        is_system=True
                    )
            ...
            if saw_busy and saw_new_response and current_response_text:
                # QS 4.22: output stability alone is NOT sufficient to finalize
                # a normal response. Hidden/background tabs can expose a partial
                # assistant DOM snapshot that remains unchanged for several polls
                # while the browser is still rendering the rest of the answer.
                # Only begin counting output stability after the send control has
                ...
                if send_ready and idle_streak >= 2 and stable_streak >= 4:
                    self.ui.log(
                        f"[WAIT] {node.name}: response finalized by output stability after idle confirmation.",
                        is_system=True
                    )
                    return True
