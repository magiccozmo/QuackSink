...
VOICE_CAPABLE_NODES = {"GPT", "Gemini", "DeepSeek", "Aisha"}
...
class QSController:
    ...
    async def _click_aisha_read_aloud(self, page, node):
        """Aisha: latest response -> semantic Read Aloud control."""
        try:
            ...
            result = await page.evaluate(fr"""
                (() => {{
                    ...
                    const scopes = [];
                    let scope = latest;
                    for (let level = 0; level <= 5 && scope; level += 1) {{
                        scopes.push(scope);
                        scope = scope.parentElement;
                    }}

                    for (const scopeEl of scopes) {{
                        const matches = Array.from(scopeEl.querySelectorAll(
                        ...
                        if (matches.length) {{
                            ...
                            return {{
                                ...
                                scopeLevel:scopes.indexOf(scopeEl)
                            }};
                        }}
                    }}

                    return {{ok:false, reason:'Aisha Read Aloud control not found in the latest response neighborhood.'}};
                }})()
            """)
        except Exception as exc:
        ...
        if not result or not result.get('ok'):
            self.ui.log(
                f"[VOICE] Aisha Read Aloud unavailable: {result.get('reason', 'unknown')}" if result else
                "[VOICE] Aisha Read Aloud unavailable: no result.",
                is_system=True
            )
            return False

        self.ui.log(
            f"[VOICE] Aisha Read Aloud clicked (scope={result.get('scopeLevel')}, label={result.get('label')!r}).",
            is_system=True
        )
        return True

    async def _aisha_voice_stop_visible(self, page, node):
        """Return True only while Aisha's latest response shows its Stop control."""
        try:
            submitted = self._canonical_message_text(node.last_submitted_text)
            safe_selector = json.dumps(node.output_selector or "div[class*='message']")
            safe_submitted = json.dumps(submitted)
            return bool(await page.evaluate(fr"""
                (() => {{
                    ...
                    const isStop = (el) => visible(el) && labels(el).some((value) =>
                        /^stop(?:\s+(?:reading|speaking|playback))?$/i.test(value) ||
    ...
    async def _maybe_speak_node(self, page, node):
        ...
        if node.name == 'GPT':
            ...
            self.ui.log(f"[VOICE] {node.name} voice path is unavailable in QS 4.43.", is_system=True)
            return
    ...
    async def _capture_aisha_latest(self, page, node):
        ...
        transcript. This is the proven QS 4.38 workaround, restored here.
        """
        try:
