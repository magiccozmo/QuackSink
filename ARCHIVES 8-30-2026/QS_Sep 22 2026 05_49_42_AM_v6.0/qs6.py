...
class QSUI:
    ...
    def _build_ui(self):
        # QS 6.0 UI MAKEOVER:
        # Everything that used to appear above BROADCAST MESSAGE lives in the
        # CONTROL PANE. BROADCAST MESSAGE and everything below it live in the
        # COMMS PANE. The panes share one vertical splitter; the control pane
        # keeps its practical working height while the comms/output pane grows
        # with the window.
        #
        # The relay/state-machine methods and widget callbacks below are kept
        # compatible with QS 5.16. Only the parent frames/layout are changed.
        ...
        self.archive_box.bind(
            "<<ComboboxSelected>>", self._on_archive_selected
        )

        try:
            self.root.option_add(
                "*TCombobox*Listbox.font", ("Arial", 18, "bold")
            )
        except Exception:
            pass
        ...
        self.comms_max_btn = tk.Button(
            comms_header,
            text="MAX COMMS",
            command=self._toggle_comms_maximize,
            bg="#555555",
            fg="white",
            font=("Arial", 10, "bold"),
            height=1,
        ...
        try:
            # Keep the control pane at a practical fixed height while the
            # comms pane absorbs added window height.
    ...
    def _toggle_comms_maximize(self):
        ...
        try:
            self.pane.add(self.control_frame, before=self.comms_frame)
            self._comms_maximized = False
