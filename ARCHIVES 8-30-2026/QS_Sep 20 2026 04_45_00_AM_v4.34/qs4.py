...
class QSController:
    ...
    async def _click_gemini_listen(self, page):
        """Gemini: latest assistant response -> More/options -> Listen."""
        try:
            opened = await page.evaluate(r"""
                (() => {
                    ...
                    const explicitMore = (value) =>
                        /^more$/i.test(value) ||
                        /^more\s+actions?$/i.test(value) ||
                        /^more\s+options?$/i.test(value) ||
                        /^(?:\.\.\.|…|⋯)$/u.test(value) ||
                        /^more\s+vert(?:ical)?$/i.test(value) ||
                        /^(?:more[_ -]?vert(?:ical)?|more[_ -]?horiz(?:ontal)?|ellipsis(?:[_ -](?:horizontal|vertical))?)$/i.test(value);
                    const isMore = (el) => {
                        if (!visible(el)) return false;
                        if (labelParts(el).some(explicitMore)) return true;

                        // Gemini can render the three-dot control as a pure icon
                        // inside a button, with no useful text/aria label on the
                        // button itself. Recognize common icon spellings and
                        // inspect the actual icon element before giving up.
                        const iconValues = Array.from(el.querySelectorAll(
                            'mat-icon,[class*="more_vert"],[class*="more-horiz"],[class*="more_vert"],[data-icon*="more"],[data-icon*="ellipsis"],svg'
                        )).flatMap((icon) => [
                            icon.textContent || '',
                        ...
                        if (iconValues.some(explicitMore)) return true;

                        // A button with a tooltip/accessible label exposed on a
                        // descendant can still be the actual ellipsis control.
                        const descendants = Array.from(el.querySelectorAll('[aria-label],[title],[data-tooltip]'));
                        return descendants.some((node) => labelParts(node).some(explicitMore));
                    };
                    ...
                    // First, search the response and a generous ancestor band.
                    // Gemini's action strip can sit outside the immediate model-response node.
                    let scope = latest;
                    let target = null;
                    for (let depth = 0; depth <= 20 && scope; depth += 1) {
                        const matches = Array.from(scope.querySelectorAll(
                            'button,[role="button"],[data-testid],[aria-label],mat-icon'
                        )).filter(isMore);
                        if (matches.length) {
                    ...
                    // Fallback: stay on this Gemini page, but choose the visible More-like
                    // control nearest to the latest response rather than searching another tab.
                    if (!target) {
                        const rr = latest.getBoundingClientRect();
                        const responseBottom = rr.bottom;
                        const pageCenterY = responseBottom;
                        const nearby = Array.from(document.querySelectorAll(
                            'button,[role="button"],[data-testid],[aria-label],mat-icon'
                        )).filter(isMore).map((el) => {
                            const r = el.getBoundingClientRect();
                            const centerY = r.top + (r.height / 2);
                            return {
                                el,
                                distance: Math.abs(centerY - pageCenterY),
                                belowBias: centerY >= responseBottom ? 0 : 1000
                            };
                        }).sort((a, b) => (a.belowBias + a.distance) - (b.belowBias + b.distance));
                        if (nearby.length) target = nearby[0].el;
                    }

                    if (!target) {
                        // Final local fallback: Gemini's ellipsis may be an icon
                        // button whose only stable identity is its SVG/material
                        // icon name. Limit this to visible buttons near the
                        // latest response so unrelated page controls cannot win.
                        const rr = latest.getBoundingClientRect();
                        const iconButtons = Array.from(document.querySelectorAll('button,[role="button"]'))
                            .filter(visible)
                            .map((el) => {
                                const r = el.getBoundingClientRect();
                                const centerY = r.top + (r.height / 2);
                                const icons = Array.from(el.querySelectorAll('mat-icon,svg,[data-icon],[data-lucide]'))
                                    .flatMap((icon) => [
                                        icon.textContent || '',
                                        icon.getAttribute('data-icon') || '',
                                        icon.getAttribute('data-lucide') || '',
                                ...
                                return { el, centerY, icons, distance: Math.abs(centerY - rr.bottom) };
                            })
                            .filter((item) => /(?:more[_ -]?vert|more[_ -]?horiz|ellipsis(?:[_ -](?:horizontal|vertical))?)/i.test(item.icons))
                            .sort((a, b) => a.distance - b.distance);
                        if (iconButtons.length) target = iconButtons[0].el;
                    }

                    if (!target) {
                        const visibleMoreLabels = Array.from(document.querySelectorAll(
                            'button,[role="button"],[data-testid],[aria-label],mat-icon'
                        )).filter(isMore).slice(0, 8).map((el) => labelParts(el).join(' | '));
                        return {
                            ok:false,
                            reason:'Gemini ellipsis/More control not found on latest response.',
                            candidates: visibleMoreLabels
                        };
                    }
        ...
        if not opened or not opened.get('ok'):
            ...
            if opened and opened.get('candidates'):
                reason += f" visible_more_candidates={opened.get('candidates')}"
            self.ui.log(f"[VOICE] Gemini More unavailable: {reason}", is_system=True)
            return False
        self.ui.log("[VOICE] Gemini More opened.", is_system=True)
        ...
        try:
            ...
            return False
        if not listened:
            self.ui.log("[VOICE] Gemini Listen menu item unavailable.", is_system=True)
            return False
