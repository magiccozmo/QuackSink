...
class QSController:
    ...
    async def _response_frame_state(self, page, node):
        ...
        if not is_new and last_message != last_baseline:
            is_new = True
        ...
        if not new_messages and last_message != last_baseline:
            new_messages = [last_message] if last_message is not None else []

        return any(str(text).strip() for text in new_messages)
    ...
    async def _wait_for_response(self, page, node, timeout=45, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            baseline = self._normalize_messages(node.response_baseline or [], "wait_baseline")
