...
class EggCagey:
    """
    Passive activity monitor for QS 3.0.

    Egg Cagey never manufactures a heartbeat.  A pulse is recorded only when
    the browser exposes a meaningful observed transition supplied by the
    caller.  The relay state machine decides what the pulse means.
    """

    def __init__(self, node_name, ui):
        self.node_name = node_name
        self.ui = ui
        self.pulses = deque(maxlen=256)
        self.last_pulse_time = None
        self.phase = "idle"

    def reset(self, phase="idle"):
        self.pulses.clear()
        self.last_pulse_time = None
        self.phase = phase

    def beat(self, event_type, detail=""):
        now = asyncio.get_event_loop().time()
        previous = self.last_pulse_time
        ...
        if interval is None:
            ...
            interval_text = f"{interval:.2f}s"
        self.ui.log(
            f"[ECG] {self.node_name}: PULSE {event_type} | since_last={interval_text}"
    ...
    def seconds_since_pulse(self):
        if self.last_pulse_time is None:
            return None
        return asyncio.get_event_loop().time() - self.last_pulse_time
...
class QSUI:
    ...
    def _build_ui(self):
        ...
        # Use a real grid row for the control strip so controls cannot collapse
        # vertically when the window/layout manager recalculates geometry.
        ctrl_bar = tk.Frame(self.root, bg="#121212")
        ctrl_bar.pack(pady=(10, 0), fill=tk.X, padx=20)
        for col in range(8):
            ctrl_bar.grid_columnconfigure(col, weight=0)

        tk.Label(ctrl_bar, text="Human Handle:", font=("Arial", 14, "bold"),
                 bg="#121212", fg="#00FFCC").grid(row=0, column=0, padx=(0, 5), sticky="w")
        ...
        # Archive actions: keep REFRESH ARCHIVES and the primary viewport/action
        # buttons on ONE physical horizontal row.
        archive_action_row = tk.Frame(archive_frame, bg="#121212")
        archive_action_row.pack(fill=tk.X, pady=2)

        tk.Button(
            archive_action_row, text="REFRESH ARCHIVES",
            command=self._refresh_archive_selector,
            bg="#555555", fg="white", font=("Arial", 10, "bold"),
            height=1, padx=8
        ).pack(side=tk.LEFT, padx=(0, 6))

        # Explicitly place these in the SAME parent row as REFRESH ARCHIVES.
        # This is intentional: they must not form a second vertical row.
        self.relay_btn = tk.Button(
            archive_action_row, text="FAN-OUT", command=self.controller.start_relay,
            bg="#444444", fg="white", font=("Arial", 16, "bold"),
            height=1, padx=15, state=tk.DISABLED
        )
        self.relay_btn.pack(side=tk.LEFT, padx=6)

        tk.Button(
            archive_action_row, text="TOP", command=self.scroll_to_top,
            bg="#0066AA", fg="white", font=("Arial", 12, "bold"),
            height=1, padx=12
        ).pack(side=tk.LEFT, padx=4)

        tk.Button(
            archive_action_row, text="BOTTOM", command=self.scroll_to_bottom,
            bg="#663399", fg="white", font=("Arial", 12, "bold"),
            height=1, padx=12
        ).pack(side=tk.LEFT, padx=4)

        # The remaining archive tools stay on their own rows.
    ...
    def _on_title_enter(self, event=None):
        self._make_title_file()
        return "break"
...
class QSController:
            ...
            async with async_playwright() as p:
                ...
                for page in context.pages:
                    ...
                    if input_sel and output_sel:
                        ...
                        node.eggcagey = EggCagey(node.name, self.ui)
                        self.nodes[node.name] = node
                        node.add_listener(self._on_node_change)
                        self.ui.log(
    ...
    async def _send_message_to_page(self, page, node, text, is_handshake):
        ...
        try:
            ...
            node.submission_user_baseline = await self._capture_user_turns(page, node)
            node.submission_payload = text
            node.input_anchor_seen = False
            node._thread_anchor_pulsed = False
            if node.eggcagey is None:
                node.eggcagey = EggCagey(node.name, self.ui)
            node.eggcagey.reset("submitting")
            self.ui.log(
            ...
            step = "SUBMIT"
            submit_success = await self._submit(page, node, input_element, selector, text)
    ...
    async def _capture_user_turns(self, page, node):
        """Return visible user-turn text using role-aware thread selectors."""
        selectors = []
        if node.user_selector:
            selectors.append(node.user_selector)
        selectors.extend([
    ...
    async def _capture_user_anchor(self, page, node, payload):
        turns = await self._capture_user_turns(page, node)
        normalized = payload.strip()
        baseline = getattr(node, "submission_user_baseline", None) or []
        baseline_count = sum(1 for turn in baseline if turn.strip() == normalized)
        current_count = sum(1 for turn in turns if turn.strip() == normalized)
        exact_new = current_count > baseline_count
        return exact_new, turns

    async def _submission_gate(self, page, node, input_element, selector, payload, method):
        """Require both composer transition and matching user-thread appearance."""
        start = asyncio.get_event_loop().time()
        input_cleared = False
        thread_seen = False
        node.submission_confirmed = False

        while asyncio.get_event_loop().time() - start < 5.0:
            await asyncio.sleep(0.25)
            try:
                input_cleared = not await self._input_has_text(page, input_element, selector)
            except Exception:
                input_cleared = False
            try:
                thread_seen, _ = await self._capture_user_anchor(page, node, payload)
            except Exception:
                thread_seen = False

            if input_cleared and not node.input_anchor_seen:
                if node.eggcagey:
                    node.eggcagey.beat("INPUT_ANCHOR", "composer cleared")
                node.input_anchor_seen = True
            if thread_seen:
                if not getattr(node, "_thread_anchor_pulsed", False):
                    if node.eggcagey:
                        node.eggcagey.beat("THREAD_ANCHOR", "submitted user turn observed")
                    node._thread_anchor_pulsed = True

            if input_cleared and thread_seen:
                node.submission_confirmed = True
                node.generation_started = False
                self.ui.log(
                    f"[SEND] SUBMIT CONFIRMED ({method}): {node.name} | "
                    f"input_clear=True thread_anchor=True",
                    is_system=True
                )
                return True
        ...
        self.ui.log(
            ...
            f"input_clear={input_cleared} thread_anchor={thread_seen}",
            is_system=True
        )
        return False

    async def _submit_verified(self, page, node, input_element, selector, method, payload):
        return await self._submission_gate(page, node, input_element, selector, payload, method)

    async def _submit(self, page, node, input_element, selector, payload):
        send_selectors = [
        ...
        for send_selector in send_selectors:
            try:
                ...
                for button in buttons:
                    ...
                    await button.click()
                    if await self._submit_verified(page, node, input_element, selector, "button", payload):
                        return True
            except Exception as exc:
                self.ui.log(
        ...
        try:
            ...
            if await self._submit_verified(page, node, input_element, selector, "Enter", payload):
                return True
        except Exception as exc:
    ...
    async def _wait_for_response(self, page, node, timeout=60, is_handshake=False):
        """Wait for generation using Egg Cagey pulses plus a hard ceiling."""
        inactivity_timeout = 20.0
        hard_ceiling = float(timeout)
        stable_required = 4
        self.ui.log(
            f"[WAIT] {node.name}: waiting for generation to finish... "
            f"stall={inactivity_timeout:.0f}s hard={hard_ceiling:.0f}s",
            is_system=True
        )

        if node.eggcagey is None:
            node.eggcagey = EggCagey(node.name, self.ui)
        node.eggcagey.phase = "waiting"
        node.generation_started = False
        ...
        last_busy = None
        last_user_anchor = False

        while True:
            now = asyncio.get_event_loop().time()
            if now - start_time >= hard_ceiling:
                self.ui.log(f"[WAIT] {node.name}: HARD TIMEOUT after {hard_ceiling:.1f}s.", is_system=True)
                return False
            if now - last_activity >= inactivity_timeout:
                self.ui.log(
                    f"[WAIT] {node.name}: GENERATION STALL timeout; "
                    f"no qualifying pulse for {now - last_activity:.1f}s.",
                    is_system=True
                )
                return False
            ...
            try:
                user_anchor, _ = await self._capture_user_anchor(page, node, node.submission_payload or "")
            except Exception:
                user_anchor = last_user_anchor
            if user_anchor and not last_user_anchor:
                node.eggcagey.beat("THREAD_ANCHOR", "new submitted user turn observed")
                last_activity = asyncio.get_event_loop().time()
            ...
            busy = not send_ready
            if busy != last_busy:
                node.eggcagey.beat("SEND_STATE_CHANGE", f"busy={busy}")
                last_activity = asyncio.get_event_loop().time()
                last_busy = busy
            ...
            if signature != last_output_signature:
                if last_output_signature is not None:
                    node.eggcagey.beat("OUTPUT_CHANGE", "clean output snapshot changed")
                    last_activity = asyncio.get_event_loop().time()
                    node.generation_started = True
                last_output_signature = signature

            if current_response_text:
                if not node.generation_started:
                    node.eggcagey.beat("GENERATION_ANCHOR", "post-submit assistant output observed")
                    last_activity = asyncio.get_event_loop().time()
                    node.generation_started = True
            ...
            if is_handshake:
                ...
                if handshake_frame:
                    ...
                    if handshake_streak >= stable_required:
                        self.ui.log(
            ...
            if current_response_text:
                ...
                if node.generation_started and stable_streak >= stable_required:
                    self.ui.log(
