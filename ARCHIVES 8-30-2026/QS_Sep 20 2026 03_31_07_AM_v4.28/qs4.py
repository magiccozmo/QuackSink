...
class QSController:
    ...
    async def _click_gpt_read_aloud(self, page):
        """Click the latest GPT response's Read Aloud control without foregrounding the tab."""
        try:
            result = await page.evaluate(r"""
                (() => {
                    ...
                    const isReadAloud = (el) =>
                        visible(el) && /read\s*aloud/i.test(labelOf(el));

                    const buttonLike = (scope) => {
                        if (!scope) return [];
                        return Array.from(scope.querySelectorAll('button,[role="button"],[data-testid]'))
                            .filter(isReadAloud);
                    };
                    ...
                    if (latest) {
                        candidates = buttonLike(latest);
                        let parent = latest.parentElement;
                        for (let i = 0; i < 5 && parent && !candidates.length; i += 1) {
                            candidates = buttonLike(parent);
                    ...
                    if (!target) {
                        return {ok: false, reason: 'Read Aloud control not found.'};
                    }
            ...
            self.ui.log(
                f"[VOICE] GPT Read Aloud probe failed: {exc}",
                is_system=True
            )
            return False
        ...
        if not result or not result.get("ok"):
            self.ui.log(
                f"[VOICE] GPT Read Aloud unavailable: {result.get('reason', 'unknown')}" if result else
                "[VOICE] GPT Read Aloud unavailable: no result.",
                is_system=True
            )
            return False

        self.ui.log(
            f"[VOICE] GPT Read Aloud clicked ({result.get('label', 'control')}).",
            is_system=True
        )
        return True
