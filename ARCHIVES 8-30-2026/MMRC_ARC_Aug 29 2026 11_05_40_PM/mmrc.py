...
# VERSION: Luma 2.6
# BASE: Luma 2.5 / Kelp / DeepSeek v14.8
# AUTHOR: Luma
# ARCHITECT: Cozmo
# PURPOSE: A bridge between minds.
#
# Luma 2.6:
#   - Reworked response detection.
#   - Response completion is now based primarily on NEW assistant output
#     appearing and becoming stable.
#   - Send-button readiness is diagnostic only, not the primary completion
#     mechanism.
#   - Streaming responses are handled by watching the newest response text
#     until it stops changing.
#   - Existing MMRC frames in conversation history cannot satisfy a new send.
#   - Unframed responses are still heard and archived but never propagated.
#   - Handshake no longer depends on UI button state.
#   - Startup archive/versioning retained.
#   - Archive selector retained.
#   - Clipboard log/source export retained.
...
logger.info("=============================================================")
logger.info("MMRC Luma 2.6 STARTING")
...
class Node:
    ...
    def __init__(self, name, url, icon="🔮"):
        ...
        # Snapshot of assistant output before the current send.
...
class MMRCUI:
    ...
    def __init__(self, controller):
        ...
        self.root.title(
            "MMRC Luma 2.6 - Stable Response Detection"
        )
...
class MMRCController:
    ...
    async def _send_message_to_page(
        ...
        try:
            ...
            # This establishes the boundary between old conversation
            # history and the response caused by THIS send.
    ...
    async def _get_new_response_candidates(
        ...
        # Normal case:
        # the assistant collection gained one or more messages.
        ...
        # Streaming clients may replace/update the final DOM node
        # rather than simply append a second node.
    ...
    async def _wait_for_response(
        ...
        """
        Wait for the response caused by the current submission.

        Luma 2.5 made Send-button readiness part of the completion
        decision. That proved too dependent on individual client UI
        behavior.

        Luma 2.6 instead watches the NEW assistant output itself.

        Completion requires:

            1. Output different from the pre-submit baseline.
            2. The newest response exists as meaningful text.
            3. That newest response stops changing for several samples.

        The Send button is sampled only for diagnostics.
        """

        self.ui.log(
            f"[WAIT] {node.name}: "
            f"watching for NEW assistant response...",
            is_system=True
        )
        ...
        observed_new_response = False

        diagnostic_busy_seen = False

        # Five identical 0.5-second samples means approximately
        # two seconds of stability.
        ...
        while (
            ...
            if not send_ready:
                diagnostic_busy_seen = True

            if candidates:

                observed_new_response = True
                ...
                if newest == last_candidate:
                    ...
                    self.ui.log(
                        f"[WAIT] {node.name}: "
                        f"new response detected "
                        f"({len(newest)} chars).",
                        is_system=True
                    )

                # A response that has stopped changing is our
            ...
            elif observed_new_response:

                # We already saw the response, but the DOM temporarily
                # stopped exposing it. Do not immediately fail.
                # Keep watching.

                pass

            # The send button can provide useful diagnostic information,
            # but it NEVER independently declares completion.

            if (
                diagnostic_busy_seen
                and
                observed_new_response
                and
                stable_count >= 3
                and
                send_ready
                ...
                self.ui.log(
                    f"[WAIT] {node.name}: "
                    f"response stable and composer ready.",
                    is_system=True
                )
        ...
        self.ui.log(
            f"[WAIT] {node.name}: "
            f"response timeout "
            f"(new_response={observed_new_response}).",
            is_system=True
        )
    ...
    async def _get_response(
        ...
        # Prefer a complete MMRC frame in the NEW response.
        ...
        # Otherwise return the newest complete text.
    ...
    def _has_complete_mmrc_frame(
        ...
        """
        Return True only when text contains a complete MMRC frame.

        The handshake itself is not treated as relay speech.
        """
