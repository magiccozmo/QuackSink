...
class QSController:
    ...
    async def _wait_for_response(self, page, node, timeout=45, is_handshake=False):
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            ...
            # completion signal. Normal relay traffic MUST NOT use the tokens
            ...
            # Normal post-handshake operation: output change alone does NOT
            # mean generation is complete. Require that generation was
            # observed busy, a new response exists, and the send control has
            # returned to ready for three consecutive polls.
            if (not is_handshake and saw_busy and saw_new_response
                    and ready_streak >= 3):
                self.ui.log(
            ...
            # Handshake fallback keeps the existing readiness-based completion
            # path available if the framed response is not detected.
            if is_handshake and saw_busy and ready_streak >= 3:
                self.ui.log(
                    f"[WAIT] {node.name}: response finalized by send readiness.",
                    is_system=True
                )
