...
#   - Uses explicit browser generation/composer state for introduction settling rather than response-scrape or Send-button assumptions.
#
# QS 4.5 RELEASE:
...
class QSController:
    ...
    async def _browser_generation_state(self, page):
        """Return explicit browser generation/composer state."""
        try:
            return await page.evaluate("""
                () => {
                    ...
                    const busyPatterns = [
                        'stop', 'cancel', 'generating', 'regenerate'
                    ];
                    ...
                    return {busyControl, composerReady};
                }
            """)
        except Exception:
            return {"busyControl": True, "composerReady": False}
    ...
    async def _wait_for_idle_after_introduction(self, page, node, timeout=90):
        ...
        while asyncio.get_event_loop().time() - start < timeout:
            ...
            if (not busy) and composer_ready:
                stable_idle += 1
                if stable_idle >= 3:
                    self.ui.log(
        ...
        self.ui.log(
            ...
            f"idle/composer-ready before timeout.",
            is_system=True
        )
        return False
