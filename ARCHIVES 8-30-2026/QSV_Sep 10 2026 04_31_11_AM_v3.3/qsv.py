# QuackSink Voice - Multi-Mind Relay Core
# VERSION: QSV 3.3
# PRODUCT: QuackSink Voice
# BASE: QSV 2.9 relay behavior
# PURPOSE: GPT Read Aloud automation for the QuackSink Voice build
#   - After each completed normal GPT relay response, QS locates the Read aloud
#     control belonging to the newest assistant response and clicks it.
#   - The behavior is limited to the GPT node; other Minds are unchanged.
#   - A failed Read aloud click is logged but never treated as a relay failure.
# BASE: QSV 2.9 / MMRC Luma 1.8
# AUTHOR: Luma
# ARCHITECT: Cozmo
...
#   - Voice starts OFF and remains unavailable until GPT is active and the installed
#     ChatGPT native Read aloud is detected from the response action menu.
#   - Uses ChatGPT's native Read aloud action in the newest GPT response.
#   - After a GPT response exists, QSV probes the response's More actions menu for
#     the native Read aloud action without activating it.
#   - When voice is ON, QSV opens that menu and clicks native Read aloud.
#   - Native Read aloud detection is independent of any browser TTS extension.
#   - Voice automation remains OFF until Cozmo explicitly enables it.
#   - A failed voice invocation never becomes a relay failure.
...
ARCHIVE_PREFIX = "QSV"
FIRST_RUN_BOOTSTRAP_POLICY = True
...
NATIVE_READ_ALOUD_TEXT = "read aloud"
NATIVE_READ_ALOUD_STOP_TEXT = "stop"
NATIVE_MORE_ACTIONS_TEXT = "more actions"
...
def _local_archives():
    ...
    for entry in os.listdir(ARCHIVE_ROOT):
        ...
        if not entry.startswith(ARCHIVE_PREFIX + "_"):
            continue
...
def verify_repository_version():
    ...
    try:
        ...
        result["message"] = (
            f"Could not hash running qsv.py: {exc}"
        )
        return result
    ...
    for name in names:
        if not name.startswith(ARCHIVE_PREFIX + "_"):
            continue
    ...
    if not matching:
        ...
        if _version_tuple(latest_version) > _version_tuple(VERSION):
            ...
            result["message"] = (
                f"Repository contains newer QSV version "
                f"{latest_version}. "
            ...
            result["message"] = (
                f"No archive found for local QSV version {VERSION}. "
                f"Newest archived QSV version found: {latest_version}. "
    ...
    for version, archive_name in matching:
        ...
        if remote_source == _read_source_bytes(local_source):
            ...
            result["message"] = (
                f"QSV {VERSION} verified: local qsv.py exactly matches "
    ...
    if successful_fetches == 0:
        ...
        result["message"] = (
            f"QSV {VERSION} archive exists, but none of the matching "
...
def startup_version_check():
    print("=" * 64)
    print(
        f"{PROGRAM_NAME} QSV {VERSION} STARTUP VERSION CHECK"
    )
    print(
    ...
    if result["status"] in ("NO_ARCHIVE", "NO_MATCHING_ARCHIVE"):
        print("-" * 64)
        print(
            f"BOOTSTRAP: No remote QSV {VERSION} archive exists yet. "
...
class QSUI:
    def __init__(self, controller):
        self.root = tk.Tk()
        self.root.title(f"QuackSink Voice QSV {VERSION} - Structural DOMAnalyzer")
    ...
    def _build_ui(self):
        tk.Label(
            self.root, text=f"QUACKSINK VOICE - MULTI-MIND RELAY CORE - QSV {VERSION}",
        ...
        self.voice_toggle_btn = tk.Button(
            ctrl_bar, text="VOICE: OFF", command=self.controller.toggle_voice,
        ...
        # Archive actions deliberately occupy separate rows so the long archive
        # selector cannot squeeze the public controls off-screen.
        ...
        for label, command, bg, padx in archive_button_specs:
            row = tk.Frame(archive_frame, bg="#121212")
            row.pack(fill=tk.X, pady=2)
            tk.Button(
                row, text=label, command=command,
    ...
    def set_voice_state(self, available, enabled):
        """Update only the voice control display. Controller owns the state."""
        try:
            ...
            if available:
                if enabled:
                    self.voice_toggle_btn.config(
                        text="VOICE: ON", bg="#00AA55", fg="white", state=tk.NORMAL
                    )
                else:
                    self.voice_toggle_btn.config(
                        text="VOICE: OFF", bg="#444444", fg="white", state=tk.NORMAL
                    )
            else:
                self.voice_toggle_btn.config(
                    text="VOICE: OFF (NATIVE READ ALOUD NOT FOUND)",
                    bg="#444444", fg="#666666", state=tk.DISABLED
                )
        except Exception:
            pass
    ...
    def _copy_selected_source(self):
        ...
        if not os.path.isfile(source_path):
            self.log(f"[CLIPBOARD] qsv.py not found: {source_path}", is_system=True)
            return
...
class QSController:
    def __init__(self):
        ...
        # Voice is a simple capability/setting pair, not a second relay state machine.
        # It always starts OFF and unavailable until the active GPT tab and the
        # installed ChatGPT native Read aloud are both known.
        self.voice_enabled = False
        self.voice_available = False
        self.voice_gpt_url = None
        self.native_read_aloud_available = False
    ...
    async def _do_scan(self):
        try:
            async with async_playwright() as p:
                ...
                # Native ChatGPT Read aloud is response-scoped. Startup discovery
                # cannot prove it exists because the response action menu is created
                # only for an actual assistant response.
                self.native_read_aloud_available = False
                self.voice_available = False
                self.voice_gpt_url = None
                self.voice_enabled = False
                self.ui.set_voice_state(False, False)
                self.ui.log(
                    "[VOICE] Native ChatGPT Read aloud will be probed after the first GPT response.",
                    is_system=True
                )
                ...
                # Do not enable voice merely because GPT is discovered. The native
                # Read aloud action belongs to an actual assistant response and is
                # verified by _refresh_voice_capability after that response completes.
                self.voice_available = False
                self.voice_enabled = False
                self.voice_gpt_url = None
                self.ui.set_voice_state(False, False)
    ...
    async def _do_handshake(self, node):
        try:
            async with async_playwright() as p:
                ...
                # --- Node state: TOGGLED -> INTRODUCING --------------------
                ...
                self.ui.log(
                    f"[INTRO] {node.name}: embedded Library preamble sent; no response required.",
                    is_system=True
                )

                # The preamble is a one-way introduction, not a request/response
                # protocol exchange.  It therefore completes the INTRODUCING
                # state immediately after successful submission.  The next
                # protocol event is the actual handshake.
                ...
                if success:
                    ...
                    if responded:
                        raw = await self._get_response(
                            page, node, is_handshake=True
                        )
                        clean, dropped = self._parse_response(
                            raw, is_handshake=True
                        )
                        if clean and not dropped:
                            ...
                            if node.name == "GPT":
                                # Native Read aloud is response-scoped; capability is
                                # deliberately not claimed during handshake.
                                self.voice_available = False
                                self.voice_enabled = False
                                self.voice_gpt_url = None
                                self.ui.set_voice_state(False, False)
                                self.ui.log(
                                    "[VOICE] GPT handshake complete. Native Read aloud will be checked after the first response.",
                                    is_system=True
                                )
                        ...
                        self.ui.log(
                            f"[HANDSHAKE] {node.name} TIMEOUT.",
                            is_system=True
                        )
                else:
                    self.ui.log(
                        f"[HANDSHAKE] {node.name} FAILED to send.",
                        is_system=True
                    )
                node.handshake_failed()
        except Exception as exc:
            ...
            async with async_playwright() as p:
                ...
                for node in self.relay.active_nodes:
                    ...
                    if responded:
                        ...
                        if clean and not dropped:
                            if node.name == "GPT":
                                await self._refresh_voice_capability(
                                    context, node, preserve_enabled=True
                                )
                                if self.voice_available and self.voice_enabled:
                                    clicked = await self._read_aloud_latest_response(page, node)
                                    if not clicked:
                                        self.voice_available = False
                                        self.voice_enabled = False
                                        self.voice_gpt_url = None
                                        self.ui.set_voice_state(False, False)
                                        self.ui.log(
                                            "[VOICE] Read aloud capability lost; voice reset to OFF.",
                                            is_system=True
                                        )
                        if dropped:
    ...
    async def _get_newest_gpt_response(self, page, node):
        """Return the newest visible GPT response element, or None."""
        if node.name != "GPT":
            return None
        try:
            result = await page.evaluate(
                ...
                (responseSelector) => {
                    ...
                    let responses = [];
                    try {
                        responses = [...document.querySelectorAll(responseSelector)];
                    } catch (e) {
                        return false;
                    }
                    ...
                    return responses.length ? responses[responses.length - 1] !== null : false;
                }
                """,
            ...
            pass
        try:
            responses = await page.query_selector_all(node.output_selector)
            visible_responses = []
            for response in responses:
                try:
                    ...
                    continue
            return visible_responses[-1] if visible_responses else None
        except Exception as exc:
            self.ui.log(
                f"[READ ALOUD] GPT: newest response lookup failed: {exc}",
                is_system=True
            )
            return None

    async def _button_matches_text(self, button, needles):
        """Check visible button metadata/text for any requested phrase."""
        try:
            ...
            for name in ("aria-label", "title", "data-testid"):
                try:
                    attrs.append((await button.get_attribute(name) or "").lower())
                except Exception:
                    pass
            try:
                attrs.append((await button.inner_text() or "").lower())
            except Exception:
                pass
            haystack = " | ".join(attrs)
    ...
    async def _find_native_read_aloud_action(self, page, node, open_menu=False):
        """Find ChatGPT's native Read aloud action for the newest response."""
        response = await self._get_newest_gpt_response(page, node)
        if not response:
            return None, None

        try:
            await response.hover(timeout=1500)
        except Exception:
            pass
        await asyncio.sleep(0.15)

        # Direct action: some ChatGPT layouts expose Read aloud directly on the response.
        try:
            buttons = await response.query_selector_all('button, [role="button"]')
            for button in buttons:
                if await self._button_matches_text(button, (NATIVE_READ_ALOUD_TEXT,)):
                    return button, None
        except Exception:
            pass

        # Find More actions from the newest response outward, but never accept a
        # page-wide match merely because its label looks right. The selected
        # control must belong to the newest response or to a nearby ancestor
        # action container demonstrably associated with that response.
        response_buttons = []
        try:
            response_buttons = await response.query_selector_all('button, [role="button"]')
        except Exception:
            response_buttons = []

        more_button = None
        for button in response_buttons:
            if await self._button_matches_text(
                button,
                (NATIVE_MORE_ACTIONS_TEXT, "more", "ellipsis", "options")
            ):
                more_button = button
                break

        # Some ChatGPT layouts render the action bar in a sibling/ancestor
        # container rather than directly inside the response node. Walk only a
        # small ancestor range and require spatial association with this exact
        # newest response. This deliberately replaces the unsafe page-wide
        # historical compatibility fallback retained from earlier QSV versions.
        if not more_button:
            ancestor = response.locator('..')
            for _ in range(4):
                try:
                    buttons = await ancestor.query_selector_all('button, [role="button"]')
                except Exception:
                    buttons = []

                for button in buttons:
                    if not await self._button_matches_text(
                        button,
                        (NATIVE_MORE_ACTIONS_TEXT, "more", "ellipsis", "options")
                    ):
                        continue
                    try:
                        associated = await page.evaluate(
                            """
                            ([responseEl, buttonEl]) => {
                                if (!responseEl || !buttonEl) return false;
                                const rr = responseEl.getBoundingClientRect();
                                const br = buttonEl.getBoundingClientRect();
                                if (!rr.width || !rr.height || !br.width || !br.height) return false;
                                const verticalGap = Math.max(0, Math.max(rr.top, br.top) - Math.min(rr.bottom, br.bottom));
                                const horizontalGap = Math.max(0, Math.max(rr.left, br.left) - Math.min(rr.right, br.right));
                                const overlapX = Math.max(0, Math.min(rr.right, br.right) - Math.max(rr.left, br.left));
                                const overlapY = Math.max(0, Math.min(rr.bottom, br.bottom) - Math.max(rr.top, br.top));
                                return responseEl.contains(buttonEl) ||
                                       (verticalGap <= 140 && horizontalGap <= Math.max(140, rr.width * 0.5)) ||
        ...
        if not more_button:
            self.ui.log(
                "[READ ALOUD] GPT: response-associated More actions control not found.",
                is_system=True
            )
            return None, None
        ...
        if open_menu:
            try:
                # Scroll the response-associated control into view, then use a
                # browser hit-test to make sure the click point actually belongs
                # to this button. Do not use force=True: an intercepted click is
                # evidence that the selected target may be wrong or covered.
                hit_test_ok = await page.evaluate(
                    """
                    (buttonEl) => {
                        if (!buttonEl) return false;
                        buttonEl.scrollIntoView({block: 'center', inline: 'nearest'});
                        const r = buttonEl.getBoundingClientRect();
                        if (!r.width || !r.height) return false;
                        const x = Math.floor(r.left + r.width / 2);
                        const y = Math.floor(r.top + r.height / 2);
                        const hit = document.elementFromPoint(x, y);
                        return hit === buttonEl || buttonEl.contains(hit);
                    }
                    """,
                    more_button
                )
                self.ui.log(
                    f"[READ ALOUD] GPT: response-associated More actions selected; hit_test_ok={bool(hit_test_ok)}",
                    is_system=True
                )
                if not hit_test_ok:
                    return None, None
                ...
                await asyncio.sleep(0.15)
                self.ui.log(
                    "[READ ALOUD] GPT: opened More actions menu for native capability/action lookup.",
                    is_system=True
                )
            except Exception as exc:
                self.ui.log(
                    f"[READ ALOUD] GPT: response-associated More actions click failed: {exc}",
                    is_system=True
                )
                return None, None

        # Search visible menu items/buttons for the native Read aloud action.
        menu_candidates = []
        try:
            menu_candidates.extend(await page.query_selector_all('[role="menuitem"], button, [role="button"]'))
        except Exception:
            pass
        for item in menu_candidates:
            if await self._button_matches_text(item, (NATIVE_READ_ALOUD_TEXT,)):
                return item, more_button

        return None, more_button

    async def _refresh_voice_capability(self, context, node, preserve_enabled=True):
        """Refresh native ChatGPT Read aloud capability without activating it."""
        if node.name != "GPT" or not node.is_active():
            self.voice_available = False
            self.voice_enabled = False
            self.voice_gpt_url = None
            self.ui.set_voice_state(False, False)
        ...
        try:
            gpt_page = None
            for candidate in context.pages:
                if node.url in (candidate.url or ""):
                    gpt_page = candidate
                    break
            if gpt_page:
                action, menu_button = await self._find_native_read_aloud_action(
                    gpt_page, node, open_menu=True
                )
                available = bool(action)
                if menu_button:
                    try:
                        await gpt_page.keyboard.press("Escape")
                    except Exception:
                        pass
        except Exception as exc:
            self.ui.log(
                f"[VOICE] Native Read aloud capability probe failed: {exc}",
                is_system=True
            )
            available = False

        self.native_read_aloud_available = available
        previous_enabled = self.voice_enabled if preserve_enabled else False
        self.voice_available = available
        self.voice_gpt_url = node.url if available else None
        self.voice_enabled = previous_enabled if available else False
        self.ui.set_voice_state(available, self.voice_enabled)

        if available:
            self.ui.log(
                "[VOICE] GPT active + ChatGPT native Read aloud FOUND. Voice toggle available.",
                is_system=True
            )
            if self.voice_enabled:
                self.ui.log("[VOICE] Voice remains ON; native capability re-verified.", is_system=True)
            else:
                self.ui.log("[VOICE] Voice is OFF until Cozmo enables it.", is_system=True)
        else:
            self.ui.log(
                "[VOICE] ChatGPT native Read aloud NOT FOUND. Voice toggle remains disabled.",
                is_system=True
            )
        return available

    async def _select_newest_gpt_response(self, page, node):
        """Select the visible text of the newest GPT assistant response."""
        if node.name != "GPT":
            return False
        try:
            ...
            self.ui.log(
                f"[READ ALOUD] GPT: response selection failed: {exc}",
                is_system=True
            )
            return False

    async def _read_aloud_latest_response(self, page, node):
        """Click ChatGPT's native Read aloud action for the newest GPT response."""
        if node.name != "GPT" or not self.voice_available or not self.voice_enabled:
            return False

        try:
            self.ui.log(
                "[READ ALOUD] GPT: locating native Read aloud action on newest response.",
                is_system=True
            )

            action, more_button = await self._find_native_read_aloud_action(
                page, node, open_menu=True
            )
            if not action:
                try:
                    ...
                    pass
                self.ui.log(
                    "[READ ALOUD] GPT: native Read aloud action not found.",
                    is_system=True
                )
                return False
            ...
            # Native ChatGPT Read aloud changes the response action into STOP while speaking.
            stop_found = False
            try:
                latest = await self._get_newest_gpt_response(page, node)
                if latest:
                    controls = await latest.query_selector_all('button, [role="button"]')
                    for control in controls:
                        if await self._button_matches_text(control, (NATIVE_READ_ALOUD_STOP_TEXT,)):
                            stop_found = True
                            break
            except Exception:
                stop_found = False
            ...
            self.ui.log(
                "[READ ALOUD] GPT: clicked ChatGPT native Read aloud action. "
                f"stop_control_detected={stop_found}",
                is_system=True
            )
            return True
        except Exception as exc:
            try:
                ...
                pass
            self.ui.log(
                f"[READ ALOUD] GPT: native Read aloud invocation failed (relay continues): {exc}",
                is_system=True
            )
            return False

    def toggle_voice(self):
        gpt = self.nodes.get("GPT")
        if not gpt or not gpt.is_active() or not self.voice_available:
            self.voice_enabled = False
            self.ui.set_voice_state(False, False)
            self.ui.log(
                "[VOICE] Toggle ignored: active GPT + ChatGPT native Read aloud capability has not been verified.",
                is_system=True
            )
            return

        self.voice_enabled = not self.voice_enabled
        self.ui.set_voice_state(True, self.voice_enabled)
        state = "ON" if self.voice_enabled else "OFF"
        self.ui.log(f"[VOICE] Voice automation toggled {state}.", is_system=True)
    ...
    async def _submit_verified(self, page, node, input_element, selector, method):
        # A successful Playwright click/keypress is only an ATTEMPT.  Confirmation
        # requires an observable UI transition: the composer clears, the node becomes
        # busy, or a new captured output message appears.
        ...
        while asyncio.get_event_loop().time() - start < 3.0:
            ...
            if not still_has_text or busy or new_output:
                self.ui.log(
    ...
    async def _submit(self, page, node, input_element, selector):
        ...
        # Try the discovered Send control first.  Do not call the attempt a success
        # until a real browser-side state transition is observed.
        ...
        # Manual testing showed Enter is a reliable DeepSeek submission path.
        # Use it as the universal verified fallback rather than assuming that a
        # click event accepted by Playwright means the application submitted.
        try:
            await input_element.focus()
        except Exception:
            pass
            ...
            return await page.evaluate(
                ...
                () => {{
                    ...
                    for (const el of elements) {{
                        ...
                        // Prefer the smallest matching element when nested selectors
                        // return both a message and one of its parent containers.
...
if __name__ == "__main__":
    logger.info("Launching QuackSink Voice QSV %s.", VERSION)
