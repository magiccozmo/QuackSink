...
class QSController:
    ...
    async def _click_gemini_listen(self, page):
        """Gemini: latest assistant response -> More -> Listen."""
        try:
            opened = await page.evaluate(r"""
                (() => {
                    ...
                    const labelOf = (el) => [el.getAttribute('aria-label') || '', el.getAttribute('title') || '', el.getAttribute('data-testid') || '', el.textContent || ''].join(' ').replace(/\s+/g, ' ').trim();
                    const isMore = (el) => {
                        if (!visible(el)) return false;
                        const aria = cleanLabel(el.getAttribute('aria-label') || '');
                        const title = cleanLabel(el.getAttribute('title') || '');
                        const testid = cleanLabel(el.getAttribute('data-testid') || '');
                        const text = cleanLabel(el.textContent || '');
                        const explicitMore = (value) =>
                            /^more$/i.test(value) ||
                            /^more\s+actions?$/i.test(value) ||
                            /^(?:\.\.\.|…|⋯)$/.test(value);
                        return explicitMore(aria) || explicitMore(title) ||
                               explicitMore(testid) || explicitMore(text);
                    };
                    const responses = Array.from(document.querySelectorAll('model-response, .model-response-text')).filter(visible);
                    let latest = responses.length ? responses[responses.length - 1] : null;
                    if (!latest) {
                        const candidates = Array.from(document.querySelectorAll('div')).filter(visible).filter((el) => /model-response/i.test(String(el.className || '')));
                    ...
                    for (let depth = 0; depth <= 7 && scope; depth += 1) {
                        const matches = Array.from(scope.querySelectorAll('button,[role="button"],[data-testid]')).filter(isMore);
                        if (matches.length) { target = matches[matches.length - 1]; break; }
                        scope = scope.parentElement;
                    }
                    if (!target) return {ok:false, reason:'Gemini More control not found on latest response.'};
                    target.click();
                    return {ok:true};
                })()
            """)
        ...
        if not opened or not opened.get('ok'):
            self.ui.log(
                f"[VOICE] Gemini More unavailable: {opened.get('reason','unknown')}" if opened else
                "[VOICE] Gemini More unavailable: no result.",
                is_system=True
            )
            return False
            ...
            listened = await page.evaluate(r"""
                (() => {
                    ...
                    const exactListen = (el) => {
                        ...
                        const aria = String(el.getAttribute('aria-label') || '').replace(/\s+/g, ' ').trim();
                        const title = String(el.getAttribute('title') || '').replace(/\s+/g, ' ').trim();
                        return /^listen$/i.test(text) || /^listen$/i.test(aria) || /^listen$/i.test(title);
                    };
                    const selectors = ['[role="menu"] [role="menuitem"]','[role="menu"] button','[role="menu"] [data-testid]','[role="menuitem"]','button,[role="button"],[data-testid]'];
                    for (const selector of selectors) {
