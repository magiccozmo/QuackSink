...
# -----------------------------------------------------------------------------
# QS 5.5 BROWSER PREFLIGHT / SELF-LAUNCH
# -----------------------------------------------------------------------------
#
...
#   3. Otherwise enumerate installed Chromium-family browsers.
#   4. Let the operator choose one in a tiny preflight dialog.
#   5. Launch that browser with a QS-owned profile and remote debugging enabled.
#   6. Verify the CDP endpoint before the normal QS window is created.
#
...
# main QS UI starts. It deliberately uses a separate profile so QS never takes
# ownership of the operator's ordinary browser session.
# -----------------------------------------------------------------------------
...
def detect_installed_browsers():
    """Find common Chromium-family browsers without depending on PATH."""
    found = {}

    # Standard Windows "App Paths" registrations catch custom/nonstandard
    # installation locations in addition to the common paths above.
    if os.name == "nt":
        try:
            import winreg
            app_keys = [
    ...
    # Common installation paths fill out the normal cases and Opera/Opera GX.
    for name, raw_path in _candidate_paths():
        exe = _existing_file(raw_path)
        if exe:
            found.setdefault(name, exe)

    # PATH fallback is useful for Chromium builds installed unusually.
    path_names = {
    ...
    # Deduplicate identical executables while retaining the friendlier browser name.
    deduped = {}
    seen = set()
...
def _show_browser_chooser(browsers):
    """Return (name, path) or None after a tiny synchronous preflight dialog."""
    if not browsers:
    ...
    result = {"name": None, "path": None, "cancelled": False}
    chooser = tk.Tk()
    chooser.title(f"QuackSink QS {VERSION} — Browser Setup")
    chooser.geometry("540x360")
    chooser.resizable(False, False)
    chooser.protocol("WM_DELETE_WINDOW", lambda: _cancel())
    ...
    ttk.Label(
        frame,
        text="No usable debug endpoint was found on port 9222.\nChoose the browser QS should launch with its own profile.",
        justify="left",
    ).pack(anchor="w", pady=(6, 14))

    selected = tk.StringVar(value=next(iter(browsers)))
    ...
    def _launch():
        ...
        if name in browsers:
            result["name"] = name
            result["path"] = browsers[name]
            chooser.destroy()

    ttk.Button(button_frame, text="Cancel", command=_cancel).pack(side="right")
    ttk.Button(button_frame, text="Launch Browser", command=_launch).pack(side="right", padx=(0, 8))
...
def _launch_browser_with_qs_profile(browser_name, executable):
    os.makedirs(BROWSER_PROFILE_ROOT, exist_ok=True)
    profile_dir = os.path.join(BROWSER_PROFILE_ROOT, _browser_slug(browser_name))
    ...
    logger.info("[BROWSER] QS-owned profile: %s", profile_dir)
    ...
    try:
        ...
        raise BrowserBootstrapError(
            f"Could not launch {browser_name}: {exc}"
        ) from exc
...
def ensure_browser_transport():
    ...
    browser_name, executable = selection
    _process, profile_dir = _launch_browser_with_qs_profile(browser_name, executable)
    ...
    while time.time() < deadline:
        ...
        if info:
            ...
            logger.info("[BROWSER] QS-owned profile ready: %s", profile_dir)
            return info
        time.sleep(0.25)

    raise BrowserBootstrapError(
        f"{browser_name} launched, but CDP did not become available on "
        f"127.0.0.1:{BROWSER_CDP_PORT} within {BROWSER_BOOTSTRAP_TIMEOUT:.0f} seconds."
    )
