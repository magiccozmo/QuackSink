...
#
# Luma 1.9 additions:
#   - Atomic response handoff: verified payload travels directly to parser.
#   - Payload-stability completion contract across consecutive polls.
#   - Removed active relay/handshake dependence on _get_response DOM refetch.
#   - Send-button readiness retained as diagnostic information, not completion authority.
#   - FAN-OUT activation scrolls relay display to top.
...
class MMRCUI:
    def __init__(self, controller):
        self.root = tk.Tk()
        self.root.title("MMRC Luma 1.9 - Stable Payload Relay")
    ...
    def _update_relay_button(self):
        ...
        # When FAN-OUT becomes available, return the relay display
        # to the top. This fires only on the inactive -> active transition.
        if now_active and not was_active:
            self.log_box.yview_moveto(0.0)

        self._fanout_active = now_active
    ...
    def _discover_archives(self):
        ...
        try:
            for entry in os.listdir(STARTUP_DIR):
                ...
                if (
                    entry.startswith(ARCHIVE_PREFIX + "_") and
                    os.path.isdir(full)
                ):
    ...
    def _copy_text_to_clipboard(
        ...
        try:
            ...
            # Keep clipboard ownership alive while MMRC runs.
            self.root.update()
    ...
    def _copy_selected_source(self):
        ...
        if not source_files:
            self.log(
                f"[CLIPBOARD] No Python source found "
                f"in {archive_path}",
                is_system=True
            )
...
class MMRCController:
        ...
        try:
            async with async_playwright() as p:
                ...
                for page in context.pages:
                    ...
                    name = SelectorDetector.detect_name_from_title(
                        title
                    )

                    if not name:
                        name = SelectorDetector.generate_name(
                            url
                        )
                    ...
                    dom_result = await DOMAnalyzer.analyze(
                        page
                    )
                    ...
                    if not input_sel or not output_sel:
                        self.ui.log(
                            f"[ANALYZE] Structural analysis incomplete "
                            f"for {name}. Falling back.",
                            is_system=True
                        )

                        fallback_input, fallback_output = (
                            SelectorDetector.get_known_selectors(
                                name
                            )
                        )
                    ...
                    if input_sel and output_sel:
                        ...
                        self.ui.log(
                            f"[DISCOVERY] Could not find selectors "
                            f"for {url}",
                            is_system=True
                        )
    ...
    async def _do_handshake(self, node):
        try:
            async with async_playwright() as p:
                ...
                if success:
                    # Luma 1.9:
                    # _wait_for_response now returns the exact verified
                    # candidate string. No second DOM extraction occurs.
                    raw = await self._wait_for_response(
                        page,
                        node,
                        timeout=45
                    )
    ...
    def start_relay(self):
        ...
        for node in active_nodes:
            ...
            self.ui.log(
                f"[RELAY] Payload enqueued for {node.name}",
                is_system=True
            )
            ...
            async with async_playwright() as p:
                ...
                for node in self.relay.active_nodes:
                    ...
                    if not node.is_active():
                        self.ui.log(
                            f"[RELAY] {node.name} not active. Skipping.",
                            is_system=True
                        )
                    ...
                    if not node.stack:
                        self.ui.log(
                            f"[RELAY] {node.name} stack empty. Skipping.",
                            is_system=True
                        )
                    ...
                    # Luma 1.9:
                    # The wait function returns the exact stable snapshot.
                    # There is deliberately NO _get_response() call here.
                    raw = await self._wait_for_response(
                        page,
                        node,
                        timeout=60
    ...
    async def _send_message_to_page(
        ...
        try:
            ...
            if (
                ...
                self.ui.log(
                    f"[SEND] LOCATE INPUT failed for "
                    f"{node.name}: '{selector}' not visible.",
                    is_system=True
                )
            ...
            self.ui.log(
                f"[SEND] FILL INPUT: {node.name} "
                f"({len(text)} chars)",
                is_system=True
            )

            # Snapshot assistant messages BEFORE submission so old MMRC
            # frames already present in the conversation cannot satisfy
            # this send.
    ...
    async def _response_frame_state(
        ...
        """
        Return the exact NEW candidate snapshot when it contains
        a complete MMRC frame.

        IMPORTANT:
        This method does not return a boolean. The returned string
        is the actual snapshot that the caller will later consume.

        That creates the atomic handoff:
            DOM snapshot -> verified candidate -> parser

        There is no second DOM extraction after completion.
        """
        ...
        # A response is new if the message collection grew or
        # the newest message changed from the pre-submit snapshot.
        ...
        # Prefer newly-added messages.
        #
        # If the newest existing message changed in place, inspect
        # that changed message instead.
        new_messages = (
        ...
        #
        # We return the COMPLETE FRAME as the candidate snapshot.
        # This is what gets compared byte-for-byte across polls.
    ...
    async def _wait_for_response(
        ...
        """
        Wait for a newly-generated MMRC response.

        Completion contract:

            candidate_t1 == candidate_t2

        The exact candidate string must remain unchanged across
        consecutive polls before it is handed to the parser.

        The send button is diagnostic only. It cannot declare a
        framed response complete.
        """
        self.ui.log(
            f"[WAIT] {node.name}: "
        ...
        while (
            ...
            # Button state remains useful as observational/diagnostic
            # information, but it is NOT completion authority.
            if not send_ready:
                if not saw_busy:
                    self.ui.log(
                        f"[WAIT] {node.name}: "
            ...
            #
            # The actual candidate payload must be identical across
            # consecutive polls.
            #
            # If the node is still streaming and the candidate changes,
            # the streak resets.
            # ----------------------------------------------------------
            if candidate is not None:
                ...
                if frame_streak >= 2:
                    self.ui.log(
                        f"[WAIT] {node.name}: "
                        f"stable NEW MMRC frame detected.",
                        is_system=True
                    )

                    # THIS is the atomic handoff.
                    #
                    # Return the exact string that was verified.
                    # No subsequent DOM query is required.
                    return candidate
            ...
            # ----------------------------------------------------------
            # SEND READINESS IS DIAGNOSTIC ONLY
            #
            # We deliberately do NOT return True here.
            #
            # A ready composer does not prove that the assistant's
            ...
            if (
                ...
                self.ui.log(
                    f"[WAIT] {node.name}: "
                    f"send-ready observed; "
                    f"still requiring payload stability.",
                    is_system=True
                )
