...
#     Read Aloud browser extension is detected.
#   - Uses the Read Aloud extension's documented browser-command path instead of
#     hunting for a ChatGPT-native Read aloud button in the GPT DOM.
#   - Before speaking, QSV selects the newest GPT response, brings that GPT tab to
#     the front, and invokes the extension's Play/Stop command (Alt+O).
...
READ_ALOUD_EXTENSION_ID = "hdhinadidafjejdhmfkjgnolgimiaplp"
READ_ALOUD_EXTENSION_URL_PREFIX = f"chrome-extension://{READ_ALOUD_EXTENSION_ID}/"
READ_ALOUD_PLAY_COMMAND = "Alt+O"
    ...
    def set_voice_state(self, available, enabled):
        try:
            ...
            if available:
                ...
                self.voice_toggle_btn.config(
                    text="VOICE: OFF (READ ALOUD EXTENSION NOT FOUND)",
...
class QSController:
    def __init__(self):
        ...
        # installed Read Aloud extension are both known.
        self.voice_enabled = False
        self.voice_available = False
        self.voice_gpt_url = None
        self.read_aloud_extension_available = False
    ...
    async def _do_scan(self):
        try:
            async with async_playwright() as p:
                ...
                self.read_aloud_extension_available = await self._detect_read_aloud_extension(
                    context.pages
                )
                if self.read_aloud_extension_available:
                    self.ui.log(
                        "[VOICE] Read Aloud browser extension DETECTED.",
                        is_system=True
                    )
                else:
                    self.ui.log(
                        "[VOICE] Read Aloud browser extension NOT DETECTED.",
                        is_system=True
                    )
                ...
                if gpt and gpt.is_active() and self.read_aloud_extension_available:
                    self.voice_available = True
                    self.voice_gpt_url = gpt.url
                    self.ui.set_voice_state(True, self.voice_enabled)
                    self.ui.log(
                        "[VOICE] GPT active + Read Aloud extension present. Voice toggle available.",
                        is_system=True
                    )
                else:
                    self.voice_available = False
                    ...
                    if responded:
                        ...
                        if clean and not dropped:
                            ...
                            if node.name == "GPT":
                                if self.read_aloud_extension_available:
                                    self.voice_available = True
                                    self.voice_gpt_url = node.url
                                    self.ui.set_voice_state(True, self.voice_enabled)
                                    self.ui.log(
                                        "[VOICE] GPT connected + Read Aloud extension present. "
                                        "VOICE toggle enabled (currently OFF).",
                                        is_system=True
                                    )
                                else:
                                    self.voice_available = False
    ...
    async def _detect_read_aloud_extension(self, pages):
        """Detect the installed Read Aloud browser extension from open extension pages."""
        for page in pages:
            try:
                url = (page.url or "").lower()
            except Exception:
                url = ""
            if READ_ALOUD_EXTENSION_ID in url:
                return True
            try:
                title = (await page.title() or "").strip().lower()
            except Exception:
                title = ""
            if title == "read aloud" and url.startswith("chrome-extension://"):
                return True
        return False

    async def _refresh_voice_capability(self, context, node, preserve_enabled=True):
        """Refresh the simple GPT + Read Aloud extension capability state."""
        ...
        available = await self._detect_read_aloud_extension(context.pages)
        self.read_aloud_extension_available = available
        ...
        if available:
            self.ui.log(
                "[VOICE] GPT active + Read Aloud extension FOUND. Voice toggle available.",
                is_system=True
            )
            if self.voice_enabled:
                self.ui.log("[VOICE] Voice remains ON; capability re-verified.", is_system=True)
            else:
                self.ui.log("[VOICE] Voice is OFF until Cozmo enables it.", is_system=True)
        else:
            self.ui.log(
                "[VOICE] Read Aloud extension NOT FOUND. Voice toggle remains disabled.",
                is_system=True
            )
        return available
    ...
    async def _read_aloud_latest_response(self, page, node):
        """Use the installed Read Aloud extension to speak the newest GPT response."""
        if node.name != "GPT" or not self.voice_available or not self.voice_enabled:
            return False

        try:
            self.ui.log(
                "[READ ALOUD] GPT: preparing newest response for browser extension.",
                is_system=True
            )
            selected = await self._select_newest_gpt_response(page, node)
            if not selected:
                return False

            await page.bring_to_front()
            await asyncio.sleep(0.15)
            # Read Aloud's current Chromium/Chrome extension registers the Play
            # command on Alt+O. With text selected, the extension reads the
            # selection rather than the whole page.
            await page.keyboard.press(READ_ALOUD_PLAY_COMMAND)
            await asyncio.sleep(0.75)
            self.ui.log(
                "[READ ALOUD] GPT: invoked installed Read Aloud extension "
                f"with {READ_ALOUD_PLAY_COMMAND} on newest response selection.",
                is_system=True
            )
            return True
        except Exception as exc:
            self.ui.log(
                f"[READ ALOUD] GPT: extension invocation failed (relay continues): {exc}",
                is_system=True
            )
            return False
    ...
    def toggle_voice(self):
        ...
        if not gpt or not gpt.is_active() or not self.voice_available:
            ...
            self.ui.set_voice_state(False, False)
            self.ui.log(
                "[VOICE] Toggle ignored: active GPT + Read Aloud extension capability has not been verified.",
                is_system=True
            )
            return
