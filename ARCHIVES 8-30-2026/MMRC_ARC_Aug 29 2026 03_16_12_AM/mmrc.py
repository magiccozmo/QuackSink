...
logger.info("=============================================================")
logger.info("MMRC Luma 1.2 STARTING")
            ...
            () => {
                ...
                for (const d of divs) {
                    if (
                        ...
                        /*
                         * IMPORTANT:
                         * Return the broad structural selector rather
                         * than constructing a selector from the current
                         * element's complete class list.
                         *
                         * This lets MMRC observe future response nodes
                         * and changing response text.
                         */
                        if (
                            d.className &&
                            ...
                            if (
                                d.className.includes('message')
                            ) {
                                candidates.push(
                                    'div[class*="message"]'
                                );
                            } else if (
                                d.className.includes('assistant')
                            ) {
                                candidates.push(
                                    'div[class*="assistant"]'
                                );
                            } else if (
                                d.className.includes('response')
                            ) {
                                candidates.push(
                                    'div[class*="response"]'
                                );
...
class MMRCUI:
    def __init__(self, controller):
        ...
        self.root.title(
            "MMRC Luma 1.2 - Structural DOMAnalyzer"
        )
    ...
    def _build_ui(self):
        tk.Label(
            self.root,
            text="MULTI-MIND RELAY CORE - Luma 1.2",
            font=("Arial", 16, "bold"),
            bg="#121212",
            fg="#00FFCC"
        ).pack(pady=(10, 0))
...
class MMRCController:
    ...
    async def _response_snapshot(
        ...
        """
        Capture the current visible response state.

        Unlike the previous implementation, this does NOT depend
        on children.length changing. A model may update the text
        inside an existing DOM element while streaming.
        """
    ...
    async def _wait_for_response(
        ...
        before = await self._response_snapshot(
            page,
            node
        )

        before_count = before.get(
            "count",
            0
        )
        ...
        while (
            ...
            after = await self._response_snapshot(
                page,
                node
            )

            after_count = after.get(
                "count",
                0
            )
            ...
            if count_changed:
                self.ui.log(
                    f"[WAIT] {node.name}: "
                    f"new response element detected.",
                    is_system=True
                )
                return True

            if text_changed and has_text:
                self.ui.log(
                    f"[WAIT] {node.name}: "
                    f"response text changed.",
                    is_system=True
                )
            ...
            if (
                ...
                final_texts = final.get(
                    "texts",
                    []
                )

                if final_texts != before_texts:
                    self.ui.log(
                        f"[WAIT] {node.name}: "
