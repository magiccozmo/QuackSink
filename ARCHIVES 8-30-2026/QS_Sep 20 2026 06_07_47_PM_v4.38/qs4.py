...
# QS 4.38 DEVELOPMENT:
#   - GPT send hardening: dismiss any transient response/action menu before composing a message.
#     This is narrowly targeted at the observed persistent More menu surviving QS shutdown/restart.
#   - Uses Escape on the GPT page after focusing the composer, then re-checks the composer before fill.
#   - Leaves the proven relay, timeout, Send Order, page identity, and voice adapters unchanged.
...
class QSController:
    ...
    async def _send_message_to_page(self, page, node, text, is_handshake):
        ...
        try:
            ...
            if node.name == "GPT":
                # GPT can retain its response More menu across QS shutdown/restart.
                # Dismiss transient UI before composing so Enter is not consumed by the menu.
                try:
                    await page.keyboard.press("Escape")
                    await asyncio.sleep(0.10)
                    self.ui.log("[SEND] GPT transient-menu dismiss (Escape)", is_system=True)
                except Exception as exc:
                    self.ui.log(f"[SEND] GPT transient-menu dismiss unavailable: {exc}", is_system=True)
                try:
                    await input_element.focus()
                except Exception:
                    pass
