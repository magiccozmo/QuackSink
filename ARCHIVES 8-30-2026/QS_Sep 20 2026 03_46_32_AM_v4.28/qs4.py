...
class QSUI:
    ...
    def _build_ui(self):
        ...
        self.luma_speak_enabled = tk.BooleanVar(value=False)
        self.luma_speak_btn = tk.Button(
            btn_frame, text="LUMA SPEAK: OFF", command=self._toggle_luma_speak,
            bg="#555555", fg="white", font=("Arial", 12, "bold"),
            height=1, padx=12
        )
        self.luma_speak_btn.pack(side=tk.LEFT, padx=4)
    ...
    def _toggle_luma_speak(self):
        enabled = not bool(self.luma_speak_enabled.get())
        self.luma_speak_enabled.set(enabled)
        if enabled:
            self.luma_speak_btn.config(text="LUMA SPEAK: ON", bg="#00AA55")
            self.log("[VOICE] Luma Speak enabled (GPT-only relays).", is_system=True)
        else:
            self.luma_speak_btn.config(text="LUMA SPEAK: OFF", bg="#555555")
            self.log("[VOICE] Luma Speak disabled.", is_system=True)

    def is_luma_speak_enabled(self):
        return bool(self.luma_speak_enabled.get())
...
class QSController:
    ...
    async def _click_gpt_read_aloud(self, page):
        """Open More Actions on the latest GPT response, then click its Read aloud menu item."""
        try:
            result = await page.evaluate(r"""
                (() => {
                    ...
                    const isMoreActions = (el) => {
                        if (!visible(el)) return false;
                        const label = labelOf(el);
                        if (/^more\s+actions?$/i.test(label)) return true;
                        if (/more\s+actions?/i.test(el.getAttribute('aria-label') || '')) return true;
                        if (/more\s+actions?/i.test(el.getAttribute('title') || '')) return true;
                        const glyph = clean(el.textContent || '');
                        return /^(?:\.\.\.|…|⋯)$/.test(glyph);
                    };
                    ...
                    if (!latest) {
                        return {ok: false, stage: 'latest_response', reason: 'Latest GPT assistant response not found.'};
                    }

                    let scope = latest;
                    let moreCandidates = [];
                    for (let depth = 0; depth <= 6 && scope; depth += 1) {
                        moreCandidates = Array.from(
                            scope.querySelectorAll('button,[role="button"],[data-testid]')
                        ).filter(isMoreActions);
                        if (moreCandidates.length) break;
                        scope = scope.parentElement;
                    }

                    const moreTarget = moreCandidates.length
                        ? moreCandidates[moreCandidates.length - 1]
                        : null;
                    ...
                    if (!moreTarget) {
                        return {
                            ok: false,
                            stage: 'more_actions',
                            reason: 'More Actions control not found on the latest GPT response.'
                        };
                    }

                    const moreLabel = labelOf(moreTarget);
                    moreTarget.click();
                    return {ok: true, stage: 'more_actions_clicked', label: moreLabel};
                })()
            """)
        except Exception as exc:
            self.ui.log(
                f"[VOICE] GPT More Actions probe failed: {exc}",
                is_system=True
            )
            return False
        ...
        self.ui.log(
            "[VOICE] GPT More Actions opened on latest assistant response.",
            is_system=True
        )

        # The menu is rendered as a portal in current ChatGPT UI, so menu lookup
        # is intentionally page-level here — but only for the exact visible
        # 'Read aloud' menu item that was exposed by the More Actions click.
        try:
            menu_item = await page.evaluate(r"""
                (() => {
                    ...
                    for (const selector of menuSelectors) {
                        candidates = Array.from(document.querySelectorAll(selector)).filter(exactReadAloud);
                        if (candidates.length) break;
                    }

                    const target = candidates.length ? candidates[candidates.length - 1] : null;
                    if (!target) {
                        return {ok: false, reason: 'Visible Read aloud menu item not found after More Actions opened.'};
                    }
            ...
            self.ui.log(
                f"[VOICE] GPT Read aloud menu probe failed: {exc}",
                is_system=True
            )
            return False

        if not menu_item or not menu_item.get("ok"):
            self.ui.log(
                f"[VOICE] GPT Read aloud menu item unavailable: {menu_item.get('reason', 'unknown')}" if menu_item else
                "[VOICE] GPT Read aloud menu item unavailable: no result.",
                is_system=True
            )
            return False
    ...
    async def _gpt_voice_stop_visible(self, page):
        """Return True while ChatGPT's voice playback Stop control is visibly present."""
        try:
            return bool(await page.evaluate(r"""
                (() => {
                    ...
                    return Array.from(
                        ...
                        if (!visible(el)) return false;
                        const label = labelOf(el);
                        return /^(stop|stop\s+reading|stop\s+reading\s+aloud)$/i.test(label) ||
                               /\bstop\s+reading\b/i.test(label);
                    });
                })()
            """))
        except Exception:
    ...
    async def _wait_for_gpt_voice(self, page, max_seconds=1800):
        """Wait for Read Aloud playback to expose and then release its Stop control."""
        start = asyncio.get_event_loop().time()
        saw_stop = False

        # First detect playback start.  A short response can finish before the
        # first sample, so probe frequently and accept a clean no-stop completion
        # only after a modest startup window.
        while asyncio.get_event_loop().time() - start < 8.0:
            if await self._gpt_voice_stop_visible(page):
                saw_stop = True
                self.ui.log("[VOICE] GPT Read Aloud playback started; waiting for Stop to disappear.", is_system=True)
                break
            await asyncio.sleep(0.2)

        if not saw_stop:
            self.ui.log("[VOICE] GPT Read Aloud clicked, but no Stop control was observed.", is_system=True)
            return False
        ...
        while asyncio.get_event_loop().time() - start < max_seconds:
            if await self._gpt_voice_stop_visible(page):
                ...
                self.ui.log("[VOICE] GPT Read Aloud playback complete; Stop control cleared.", is_system=True)
                return True
            await asyncio.sleep(0.5)

        self.ui.log("[VOICE] GPT Read Aloud safety timeout reached.", is_system=True)
        return False

    async def _maybe_speak_gpt(self, page, node):
        """Speak only a completed GPT response in a GPT-only relay when enabled."""
        if node.name != "GPT":
            return
        if not self.ui.is_luma_speak_enabled():
            return
        if len(self.relay.active_nodes) != 1 or self.relay.active_nodes[0] != node:
            self.ui.log(
                "[VOICE] Luma Speak skipped: GPT is not the only active relay node.",
                is_system=True
            )
            return
        ...
        clicked = await self._click_gpt_read_aloud(page)
        if not clicked:
            return
        await self._wait_for_gpt_voice(page)
            ...
            for node in self.relay.active_nodes:
                ...
                if responded:
                    ...
                    if clean:
                        ...
                        self.ui.log(f"\n{relay_msg}\n")
                        await self._maybe_speak_gpt(page, node)
